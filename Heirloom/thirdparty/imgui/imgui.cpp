...
ImGuiStyle::ImGuiStyle()
{
    ...
    WindowRounding =
        7.0f;             // Radius of window corners rounding. Set to 0.0f to have rectangular windows. Large values tend to lead to variety of artifacts and are not recommended.
    WindowBorderSize =
...
void ImGuiStyle::ScaleAllSizes(float scale_factor)
{
    ...
    if (TabMinWidthForUnselectedCloseButton != FLT_MAX) TabMinWidthForUnselectedCloseButton = ImFloor(
...
{
    if (c != 0) InputQueueCharacters.push_back(c <= IM_UNICODE_CODEPOINT_MAX
...
void ImGuiIO::AddInputCharacterUTF16(ImWchar16 c)
{
    ...
    if (InputQueueSurrogate != 0)
    {
        ...
        else if constexpr (IM_UNICODE_CODEPOINT_MAX == (0xFFFF)
        ...
        else cp = static_cast<ImWchar>(((InputQueueSurrogate - 0xD800) << 10) + (c - 0xDC00) + 0x10000);
...
void ImGuiIO::AddInputCharactersUTF8(const char* utf8_chars)
{
    while (*utf8_chars != 0)
    {
        ...
        if (c != 0) InputQueueCharacters.push_back(static_cast<ImWchar>(c));
    }
}
...
void* ImFileLoadToMemory(const char* filename, const char* mode, size_t* out_file_size, int padding_bytes)
{
    ...
    if (padding_bytes > 0) memset(static_cast<void*>(static_cast<char*>(file_data) + file_size),
                                  0,
...
{
    unsigned int c           = static_cast<unsigned>(-1);
    const unsigned char* str = (const unsigned char*)in_text;
    if (!(*str & 0x80))
    {
        c         = static_cast<unsigned>(*str++);
        *out_char = c;
        return 1;
    }
    if ((*str & 0xe0) == 0xc0)
    {
        *out_char = IM_UNICODE_CODEPOINT_INVALID; // will be invalid but not end of string
        ...
        c = static_cast<unsigned>((*str++ & 0x1f) << 6);
    ...
    if ((*str & 0xf0) == 0xe0)
    {
        *out_char = IM_UNICODE_CODEPOINT_INVALID; // will be invalid but not end of string
        ...
        c = static_cast<unsigned>((*str++ & 0x0f) << 12);
        if ((*str & 0xc0) != 0x80) return 3;
        c += static_cast<unsigned>((*str++ & 0x3f) << 6);
        if ((*str & 0xc0) != 0x80) return 3;
        c += (*str++ & 0x3f);
        *out_char = c;
        return 3;
    ...
    if ((*str & 0xf8) == 0xf0)
    {
        *out_char = IM_UNICODE_CODEPOINT_INVALID; // will be invalid but not end of string
        ...
        c = static_cast<unsigned>((*str++ & 0x07) << 18);
        if ((*str & 0xc0) != 0x80) return 4;
        c += static_cast<unsigned>((*str++ & 0x3f) << 12);
        if ((*str & 0xc0) != 0x80) return 4;
        c += static_cast<unsigned>((*str++ & 0x3f) << 6);
        if ((*str & 0xc0) != 0x80) return 4;
        c += (*str++ & 0x3f);
...
int ImTextStrToUtf8(char* buf, int buf_size, const ImWchar* in_text, const ImWchar* in_text_end)
{
    ...
    while (buf_out < buf_end - 1 && (!in_text_end || in_text < in_text_end) && *in_text)
    {
        ...
        if (c < 0x80) *buf_out++ = static_cast<char>(c);
        else buf_out += ImTextCharToUtf8(buf_out, static_cast<int>(buf_end - buf_out - 1), c);
    }
    *buf_out = 0;
    return static_cast<int>(buf_out - buf);
}
...
IMGUI_API ImU32 ImAlphaBlendColors(ImU32 col_a, ImU32 col_b)
{
    ...
    int r   = ImLerp(static_cast<int>(col_a >> IM_COL32_R_SHIFT) & 0xFF,
                     static_cast<int>(col_b >> IM_COL32_R_SHIFT) & 0xFF,
                     t);
    int g = ImLerp(static_cast<int>(col_a >> IM_COL32_G_SHIFT) & 0xFF,
                   static_cast<int>(col_b >> IM_COL32_G_SHIFT) & 0xFF,
                   t);
    int b = ImLerp(static_cast<int>(col_a >> IM_COL32_B_SHIFT) & 0xFF,
                   static_cast<int>(col_b >> IM_COL32_B_SHIFT) & 0xFF,
                   t);
...
void ImGuiStorage::SetAllInt(int v) { for (int i = 0; i < Data.Size; i++) Data[i].val_i = v; }
                             ...
                             int* out_items_display_end)
{
    ...
    if (g.NavJustMovedToId && window->NavLastIds[0] == g.NavJustMovedToId) unclipped_rect.Add(
...
static void SetCursorPosYAndSetupForPrevLine(float pos_y, float line_height)
{
    ...
    if (ImGuiColumns* columns = window->DC.CurrentColumns) columns->LineMinY = window->DC.CursorPos.
...
bool ImGuiListClipper::Step()
{
    ...
    if (StepNo == 2
    ) // Step 2: empty step only required if an explicit items_height was passed to constructor or Begin() and user still call Step(). Does nothing and switch to Step 3.
    {
...
static const ImGuiStyleVarInfo GStyleVarInfo[] = {
    {ImGuiDataType_Float, 1, static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, Alpha))},               // ImGuiStyleVar_Alpha
    {
        ImGuiDataType_Float,
        2,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, WindowPadding))
    },       // ImGuiStyleVar_WindowPadding
    {
        ImGuiDataType_Float,
        1,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, WindowRounding))
    },      // ImGuiStyleVar_WindowRounding
    {
        ImGuiDataType_Float,
        1,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, WindowBorderSize))
    },    // ImGuiStyleVar_WindowBorderSize
    {
        ImGuiDataType_Float,
        2,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, WindowMinSize))
    },       // ImGuiStyleVar_WindowMinSize
    {
        ImGuiDataType_Float,
        2,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, WindowTitleAlign))
    },    // ImGuiStyleVar_WindowTitleAlign
    {
        ImGuiDataType_Float,
        1,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, ChildRounding))
    },       // ImGuiStyleVar_ChildRounding
    {
        ImGuiDataType_Float,
        1,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, ChildBorderSize))
    },     // ImGuiStyleVar_ChildBorderSize
    {
        ImGuiDataType_Float,
        1,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, PopupRounding))
    },       // ImGuiStyleVar_PopupRounding
    {
        ImGuiDataType_Float,
        1,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, PopupBorderSize))
    },     // ImGuiStyleVar_PopupBorderSize
    {
        ImGuiDataType_Float,
        2,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, FramePadding))
    },        // ImGuiStyleVar_FramePadding
    {
        ImGuiDataType_Float,
        1,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, FrameRounding))
    },       // ImGuiStyleVar_FrameRounding
    {
        ImGuiDataType_Float,
        1,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, FrameBorderSize))
    },     // ImGuiStyleVar_FrameBorderSize
    {
        ImGuiDataType_Float,
        2,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, ItemSpacing))
    },         // ImGuiStyleVar_ItemSpacing
    {
        ImGuiDataType_Float,
        2,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, ItemInnerSpacing))
    },    // ImGuiStyleVar_ItemInnerSpacing
    {
        ImGuiDataType_Float,
        1,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, IndentSpacing))
    },       // ImGuiStyleVar_IndentSpacing
    {
        ImGuiDataType_Float,
        1,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, ScrollbarSize))
    },       // ImGuiStyleVar_ScrollbarSize
    {
        ImGuiDataType_Float,
        1,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, ScrollbarRounding))
    },   // ImGuiStyleVar_ScrollbarRounding
    {
        ImGuiDataType_Float,
        1,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, GrabMinSize))
    },         // ImGuiStyleVar_GrabMinSize
    {
        ImGuiDataType_Float,
        1,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, GrabRounding))
    },        // ImGuiStyleVar_GrabRounding
    {
        ImGuiDataType_Float,
        1,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, TabRounding))
    },         // ImGuiStyleVar_TabRounding
    {
        ImGuiDataType_Float,
        2,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, ButtonTextAlign))
    },     // ImGuiStyleVar_ButtonTextAlign
    {
        ImGuiDataType_Float,
        2,
        static_cast<ImU32>(IM_OFFSETOF(ImGuiStyle, SelectableTextAlign))
                               ...
                               const ImVec2* text_size_if_known)
{
    ...
    else RenderTextClippedEx(draw_list,
                             pos_min,
                             ImVec2(clip_max_x, pos_max.y),
                             text,
                             text_end_full,
...
void ImGui::RenderNavHighlight(const ImRect& bb, ImGuiID id, ImGuiNavHighlightFlags flags)
{
    ...
    if (flags & ImGuiNavHighlightFlags_TypeThin) window->DrawList->AddRect(
        display_rect.Min,
        display_rect.Max,
        GetColorU32(ImGuiCol_NavHighlight),
        rounding,
...
ImGuiID ImGuiWindow::GetIDFromRectangle(const ImRect& r_abs)
{
    ImGuiID seed       = IDStack.back();
    const int r_rel[4] = {
        static_cast<int>(r_abs.Min.x - Pos.x),
        static_cast<int>(r_abs.Min.y - Pos.y),
        static_cast<int>(r_abs.Max.x - Pos.x),
        static_cast<int>(r_abs.Max.y - Pos.y)
    };
        ...
        if (ImGuiWindow* focused_root_window = g.NavWindow->RootWindow)
            if (focused_root_window->WasActive && focused_root_window != window->RootWindow)
            {
                ...
                if ((focused_root_window->Flags & ImGuiWindowFlags_Popup) && !(flags &
                    ImGuiHoveredFlags_AllowWhenBlockedByPopup)) return false;
            }

    // Filter by viewport
    if (window->Viewport != g.MouseViewport)
        if (g.MovingWindow == nullptr || window->RootWindow != g.MovingWindow->RootWindow) return false;
...
bool ImGui::IsItemHovered(ImGuiHoveredFlags flags)
{
    ...
    if ((window->DC.ItemFlags & ImGuiItemFlags_Disabled) && !(flags & ImGuiHoveredFlags_AllowWhenDisabled)) return
        false;
...
bool ImGui::ItemHoverable(const ImRect& bb, ImGuiID id)
{
    ...
    if (id != 0)
    {
        ...
        if (g.DebugItemPickerActive && g.HoveredIdPreviousFrame == id) GetForegroundDrawList()->AddRect(
            bb.Min,
            bb.Max,
...
float ImGui::CalcWrapWidthForPos(const ImVec2& pos, float wrap_pos_x)
{
    ...
    else if (wrap_pos_x > 0.0f) wrap_pos_x += window->Pos.x - window->Scroll.
...
void ImGui::UpdateMouseMovingWindowNewFrame()
{
    ...
    if (g.MovingWindow != nullptr)
    {
        ...
        else
        {
            ...
            if (g.ConfigFlagsCurrFrame & ImGuiConfigFlags_ViewportsEnable) UpdateTryMergeWindowIntoHostViewport(
                moving_window,
                g.MouseViewport);
...
void ImGui::UpdateMouseMovingWindowEndFrame()
{
    ...
    if (g.IO.MouseClicked[1])
    {
        ...
        if (modal == nullptr) hovered_window_above_modal = true;
...
static void ImGui::UpdateMouseInputs()
{
    ...
    if (IsMousePosValid(&g.IO.MousePos) && IsMousePosValid(&g.IO.MousePosPrev)) g.IO.MouseDelta = g.IO.MousePos - g.IO.
        MousePosPrev;
        ...
        if (g.IO.MouseClicked[i])
        {
            if (static_cast<float>(g.Time - g.IO.MouseClickedTime[i]) < g.IO.MouseDoubleClickTime)
            {
                ImVec2 delta_from_click_pos = IsMousePosValid(&g.IO.MousePos)
                                                  ...
                                                  : ImVec2(0.0f, 0.0f);
                if (ImLengthSqr(delta_from_click_pos) < g.IO.MouseDoubleClickMaxDist * g.IO.MouseDoubleClickMaxDist) g.
...
void ImGui::UpdateMouseWheel()
{
    ...
    if (g.WheelingWindow != nullptr)
    {
        ...
        if (IsMousePosValid() && ImLengthSqr(g.IO.MousePos - g.WheelingWindowRefMousePos) > g.IO.MouseDragThreshold * g.
            IO.MouseDragThreshold) g.WheelingWindowTimer = 0.0f;
    ...
    if (wheel_y != 0.0f && !g.IO.KeyCtrl)
    {
        ...
        while ((window->Flags & ImGuiWindowFlags_ChildWindow) && ((window->ScrollMax.y == 0.0f) || ((window->Flags &
            ImGuiWindowFlags_NoScrollWithMouse) && !(window->Flags & ImGuiWindowFlags_NoMouseInputs)))) window = window
            ->ParentWindow;
...
void ImGui::UpdateTabFocus()
{
    ...
    if (g.ActiveId == 0 && g.FocusTabPressed)
    {
        ...
        if (g.NavId != 0 && g.NavIdTabCounter != INT_MAX) g.FocusRequestNextCounterTabStop = g.NavIdTabCounter + 1 + (
            g.IO.KeyShift ? -1 : 1);
    ...
    if (g.FocusRequestNextWindow != nullptr)
    {
        ...
        if (g.FocusRequestNextCounterRegular != INT_MAX && window->DC.FocusCounterRegular != -1) g.
            FocusRequestCurrCounterRegular = ImModPositive(g.FocusRequestNextCounterRegular,
                                                           window->DC.FocusCounterRegular + 1);
        if (g.FocusRequestNextCounterTabStop != INT_MAX && window->DC.FocusCounterTabStop != -1) g.
            FocusRequestCurrCounterTabStop = ImModPositive(g.FocusRequestNextCounterTabStop,
...
void ImGui::UpdateHoveredWindowAndCaptureFlags()
{
    ...
    for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++)
    {
        if (g.IO.MouseClicked[i]) g.IO.MouseDownOwned[i] = (g.HoveredWindow != nullptr) || (g.OpenPopupStack.Size > 0);
        mouse_any_down |= g.IO.MouseDown[i];
        if (g.IO.MouseDown[i])
            if (mouse_earliest_button_down == -1 || g.IO.MouseClickedTime[i] < g.IO.MouseClickedTime[
                mouse_earliest_button_down]) mouse_earliest_button_down = i;
    }
    ...
    if (clear_hovered_windows) g.HoveredWindow = g.HoveredRootWindow = g.HoveredWindowUnderMovingWindow = nullptr;

    // Update io.WantCaptureMouse for the user application (true = dispatch mouse info to imgui, false = dispatch mouse info to Dear ImGui + app)
    if (g.WantCaptureMouseNextFrame != -1) g.IO.WantCaptureMouse = (g.WantCaptureMouseNextFrame != 0);
    else g.IO.WantCaptureMouse = (mouse_avail_to_imgui && (g.HoveredWindow != nullptr || mouse_any_down)) || (g.
        OpenPopupStack.Size > 0);

    // Update io.WantCaptureKeyboard for the user application (true = dispatch keyboard info to imgui, false = dispatch keyboard info to Dear ImGui + app)
    if (g.WantCaptureKeyboardNextFrame != -1) g.IO.WantCaptureKeyboard = (g.WantCaptureKeyboardNextFrame != 0);
    else g.IO.WantCaptureKeyboard                                      = (g.ActiveId != 0) || (modal_window != nullptr);
    if (g.IO.NavActive && (g.IO.ConfigFlags & ImGuiConfigFlags_NavEnableKeyboard) && !(g.IO.ConfigFlags &
        ImGuiConfigFlags_NavNoCaptureKeyboard)) g.IO.WantCaptureKeyboard = true;
...
void ImGui::NewFrame()
{
    ...
    g.DrawListSharedData.ClipRectFullscreen = ImVec4(virtual_space.Min.x,
                                                     virtual_space.Min.y,
                                                     virtual_space.Max.x,
                                                     virtual_space.Max.y);
    ...
    if (g.Style.AntiAliasedLinesUseTex && !(g.Font->ContainerAtlas->Flags & ImFontAtlasFlags_NoBakedLines)) g.
        DrawListSharedData.InitialFlags |= ImDrawListFlags_AntiAliasedLinesUseTex;
    if (g.Style.AntiAliasedFill) g.DrawListSharedData.InitialFlags |= ImDrawListFlags_AntiAliasedFill;
    if (g.IO.BackendFlags & ImGuiBackendFlags_RendererHasVtxOffset) g.DrawListSharedData.InitialFlags |=
    ...
    for (int i = 0; i < IM_ARRAYSIZE(g.IO.KeysDown); i++) g.IO.KeysDownDuration[i] = g.IO.KeysDown[i]
    ...
    if (GetTopMostPopupModal() != nullptr || (g.NavWindowingTarget != nullptr && g.NavWindowingHighlightAlpha > 0.0f)) g
        .DimBgRatio   = ImMin(g.DimBgRatio + g.IO.DeltaTime * 6.0f, 1.0f);
    ...
    const float memory_compact_start_time = (g.IO.ConfigWindowsMemoryCompactTimer >= 0.0f)
                                                ? static_cast<float>(g.Time) - g.IO.ConfigWindowsMemoryCompactTimer
                                                : FLT_MAX;
...
static void SetupViewportDrawData(ImGuiViewportP* viewport, ImVector<ImDrawList*>* draw_lists)
{
    ...
    draw_data->FramebufferScale = ImGui::GetIO().
        DisplayFramebufferScale; // FIXME-VIEWPORT: This may vary on a per-monitor/viewport basis?
...
static ImGuiWindow* FindFrontMostVisibleChildWindow(ImGuiWindow* window)
{
    for (int n = window->DC.ChildWindows.Size - 1; n >= 0; n--)
        if (IsWindowActiveAndVisible(window->DC.ChildWindows[n])) return FindFrontMostVisibleChildWindow(
            window->DC.ChildWindows[n]);
    return window;
}
...
static void ImGui::EndFrameDrawDimmedBackgrounds()
{
    ...
    if (dim_bg_for_window_list && g.NavWindowingTargetAnim->Active)
    {
        ...
        if (window->RootWindowDockStop->DockIsActive)
            if (window->RootWindow != window->RootWindowDockStop) RenderRectFilledWithHole(
                draw_list,
...
void ImGui::EndFrame()
{
    ...
    if (g.PlatformIO.Platform_SetImeInputPos && (g.PlatformImeLastPos.x == FLT_MAX || ImLengthSqr(
...
void ImGui::Render()
{
    ...
    for (int n = 0; n != g.Viewports.Size; n++)
    {
        ...
        if (viewport->DrawLists[0] != nullptr) AddDrawListToDrawData(&viewport->DrawDataBuilder.Layers[0],
    ...
    for (int n = 0; n != g.Windows.Size; n++)
    {
        ...
        if (IsWindowActiveAndVisible(window) && (window->Flags & ImGuiWindowFlags_ChildWindow) == 0 && window !=
            windows_to_render_top_most[0] && window != windows_to_render_top_most[1]) AddRootWindowToDrawData(window);
    }
    ...
    if (g.IO.MouseDrawCursor && g.MouseCursor != ImGuiMouseCursor_None) g.IO.Fonts->GetMouseCursorTexData(
        g.MouseCursor,
        &mouse_cursor_offset,
        &mouse_cursor_size,
        &mouse_cursor_uv[0],
    ...
    for (int n = 0; n < g.Viewports.Size; n++)
    {
        ...
        if (mouse_cursor_size.x > 0.0f && mouse_cursor_size.y > 0.0f)
        {
            ...
            if (viewport->GetMainRect().Overlaps(ImRect(g.IO.MousePos,
                                                        g.IO.MousePos + ImVec2(
                                                            mouse_cursor_size.x + 2,
                                                            mouse_cursor_size.y + 2) * scale))) RenderMouseCursor(
        ...
        if (viewport->DrawLists[1] != nullptr) AddDrawListToDrawData(&viewport->DrawDataBuilder.Layers[0],
                                                                     GetForegroundDrawList(viewport));
    ...
    #ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    if (g.Viewports[0]->DrawData->CmdListsCount > 0 && g.IO.RenderDrawListsFn != nullptr) g.IO.RenderDrawListsFn(
        g.Viewports[0]->DrawData);
    #endif
}
...
ImVec2 ImGui::CalcTextSize(const char* text, const char* text_end, bool hide_text_after_double_hash, float wrap_width)
{
    ...
    const char* text_display_end;
    if (hide_text_after_double_hash) text_display_end =
...
static void FindHoveredWindow()
{
    ...
    for (int i = g.Windows.Size - 1; i >= 0; i--)
    {
        ...
        if (window->Flags & (ImGuiWindowFlags_ChildWindow | ImGuiWindowFlags_NoResize |
            ImGuiWindowFlags_AlwaysAutoResize)) bb.Expand(padding_regular);
        ...
        if (hovered_window_ignoring_moving_window == nullptr && (!g.MovingWindow || window->RootWindow != g.MovingWindow
            ->RootWindow)) hovered_window_ignoring_moving_window = window;
...
bool ImGui::IsKeyPressed(int user_key_index, bool repeat)
{
    ...
    if (repeat && t > g.IO.KeyRepeatDelay) return GetKeyPressedAmount(user_key_index,
                                                                      g.IO.KeyRepeatDelay,
...
ImVec2 ImGui::GetMouseDragDelta(ImGuiMouseButton button, float lock_threshold)
{
    ...
    if (g.IO.MouseDown[button] || g.IO.MouseReleased[button])
        if (g.IO.MouseDragMaxDistanceSqr[button] >= lock_threshold * lock_threshold)
            if (IsMousePosValid(&g.IO.MousePos) && IsMousePosValid(&g.IO.MouseClickedPos[button])) return g.IO.MousePos
...
bool ImGui::IsItemDeactivated()
{
    ImGuiContext& g     = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_HasDeactivated) return (window->DC.LastItemStatusFlags &
...
bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    const int auto_fit_axises  = ((size.x == 0.0f) ? (1 << ImGuiAxis_X) : 0x00) | ((size.y == 0.0f)
        ? (1 << ImGuiAxis_Y)
        : 0x00);
    if (size.x <= 0.0f) size.x =
        ImMax(content_avail.x + size.x, 4.0f); // Arbitrary minimum child size (0.0f causing too much issues)
    if (size.y <= 0.0f) size.y = ImMax(content_avail.y + size.y, 4.0f);
    SetNextWindowSize(size);

    // Build up name. If you need to append to a same child from multiple location in the ID stack, use BeginChild(ImGuiID id) with a stable value.
    char title[256];
    if (name) ImFormatString(title, IM_ARRAYSIZE(title), "%s/%s_%08X", parent_window->Name, name, id);
    else ImFormatString(title, IM_ARRAYSIZE(title), "%s/%08X", parent_window->Name, id);
...
void ImGui::EndChild()
{
    ...
    else
    {
            ...
            ImGuiWindowFlags_NavFlattened))
        {
            ...
            if (window->DC.NavLayerActiveMask == 0 && window == g.NavWindow) RenderNavHighlight(
...
static void ApplyWindowSettings(ImGuiWindow* window, ImGuiWindowSettings* settings)
{
    ...
    if (settings->Size.x > 0 && settings->Size.y > 0) window->Size = window->SizeFull = ImFloor(
...
static ImGuiWindow* CreateNewWindow(const char* name, ImGuiWindowFlags flags)
{
    ...
    window->DC.CursorStartPos = window->DC.CursorMaxPos = window->
    ...
    if (flags & ImGuiWindowFlags_NoBringToFrontOnFocus) g.Windows.
                                                          push_front(window); // Quite slow but rare and only once
...
static ImVec2 CalcWindowContentSize(ImGuiWindow* window)
{
    ...
    if (window->Hidden && window->HiddenFramesCannotSkipItems == 0 && window->HiddenFramesCanSkipItems > 0) return
        window->ContentSize;
...
static ImVec2 CalcWindowAutoFitSize(ImGuiWindow* window, const ImVec2& size_contents)
{
    ...
    if (monitor_idx >= 0 && monitor_idx < g.PlatformIO.Monitors.Size) avail_size = g.PlatformIO.Monitors[monitor_idx].
        WorkSize;
...
static ImRect GetResizeBorderRect(ImGuiWindow* window, int border_n, float perp_padding, float thickness)
{
    ...
    if (border_n == 0) return ImRect(rect.Min.x + perp_padding,
                                     rect.Min.y - thickness,
                                     rect.Max.x - perp_padding,
                                     rect.Min.y + thickness);
    // Top
    if (border_n == 1) return ImRect(rect.Max.x - thickness,
                                     rect.Min.y + perp_padding,
                                     rect.Max.x + thickness,
                                     rect.Max.y - perp_padding);
    // Right
    if (border_n == 2) return ImRect(rect.Min.x + perp_padding,
                                     rect.Max.y - thickness,
                                     rect.Max.x - perp_padding,
                                     rect.Max.y + thickness);
    // Bottom
    if (border_n == 3) return ImRect(rect.Min.x - thickness,
                                     rect.Min.y + perp_padding,
                                            ...
                                            const ImRect& visibility_rect)
{
    ...
    if ((flags & ImGuiWindowFlags_NoResize) || (flags & ImGuiWindowFlags_AlwaysAutoResize) || window->AutoFitFramesX > 0
        || window->AutoFitFramesY > 0) return false;
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++)
    {
        ...
        if (hovered || held) g.MouseCursor = (resize_grip_n & 1)
        ...
        if (resize_grip_n == 0 || held || hovered) resize_grip_col[resize_grip_n] = GetColorU32(
    ...
    if (g.NavWindowingTarget && g.NavWindowingTarget->RootWindow == window)
    {
        ...
        if (g.NavInputSource == ImGuiInputSource_NavKeyboard && g.IO.KeyShift) nav_resize_delta = GetNavInputAmount2d(
            ImGuiNavDirSourceFlags_Keyboard,
            ImGuiInputReadMode_Down);
        if (g.NavInputSource == ImGuiInputSource_NavGamepad) nav_resize_delta = GetNavInputAmount2d(
...
static inline void ClampWindowRect(ImGuiWindow* window, const ImRect& visibility_rect)
{
    ...
    if (g.IO.ConfigWindowsMoveFromTitleBarOnly && !(window->Flags & ImGuiWindowFlags_NoTitleBar)) size_for_clamping.y =
...
static void ImGui::RenderWindowOuterBorders(ImGuiWindow* window)
{
    ...
    if (border_size > 0.0f && !(window->Flags & ImGuiWindowFlags_NoBackground)) window->DrawList->AddRect(
        window->Pos,
    ...
    if (border_held != -1)
    {
        ...
        window->DrawList->PathArcTo(
            ...
            def.OuterAngle + IM_PI * 0.25f);
        window->DrawList->
                PathStroke(GetColorU32(ImGuiCol_SeparatorActive),
                           false,
                                    ...
                                    float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        if (!(flags & ImGuiWindowFlags_NoBackground))
        {
            ...
            if (g.DragDropActive && (g.FrameCount - g.DragDropAcceptFrameCount) <= 1 && g.IO.
                ...
                if (g.DragDropPayload.IsDataType(IMGUI_PAYLOAD_TYPE_WINDOW) && *static_cast<ImGuiWindow**>(g.
                    DragDropPayload.Data) == window) is_docking_transparent_payload = true;
            ...
            else
            {
                ...
                if (override_alpha) bg_col = (bg_col & ~IM_COL32_A_MASK) | (IM_F32_TO_INT8_SAT(alpha) <<
                    IM_COL32_A_SHIFT);
            }
        ...
        if (flags & ImGuiWindowFlags_MenuBar)
        {
            ...
            menu_bar_rect.
                ClipWith(window->Rect());  // Soft clipping, in particular child window don't have minimum size covering the menu bar so this is useful for them.
            window->DrawList->AddRectFilled(menu_bar_rect.Min + ImVec2(window_border_size, 0),
                                            ...
                                            ImDrawCornerFlags_Top);
            if (style.FrameBorderSize > 0.0f && menu_bar_rect.Max.y < window->Pos.y + window->Size.y) window->DrawList->
                AddLine(menu_bar_rect.GetBL(),
                        menu_bar_rect.GetBR(),
                        GetColorU32(ImGuiCol_Border),
        ...
        if (window->DockIsActive && node->IsHiddenTabBar() && !node->IsNoTabBar())
        {
            ...
            bool hovered, held;
            if (ButtonBehavior(r, window->GetID("#UNHIDE"), &hovered, &held, ImGuiButtonFlags_FlattenChildren)) node->
                WantHiddenTabBarToggle = true;
            else if (held && IsMouseDragging(0)) StartMouseMovingWindowOrNode(window, node, true);
            ...
            ImU32 col = GetColorU32(((held && hovered) || (node->IsFocused && !hovered))
                                        ...
                                        : hovered
                                              ...
                                              : ImGuiCol_Button);
            window->DrawList->AddTriangleFilled(p,
                                         ...
                                         bool* p_open)
{
    ...
    if (has_collapse_button)
        if (CollapseButton(window->GetID("#COLLAPSE"), collapse_button_pos, nullptr)) window->WantCollapseToggle =
...
void ImGui::UpdateWindowParentAndRootLinks(ImGuiWindow* window, ImGuiWindowFlags flags, ImGuiWindow* parent_window)
{
    ...
    if (parent_window && (flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Tooltip))
    {
        ...
        if (!window->DockIsActive && !(parent_window->Flags & ImGuiWindowFlags_DockNodeHost)) window->RootWindowDockStop
            = parent_window->RootWindowDockStop;
    }
    if (parent_window && !(flags & ImGuiWindowFlags_Modal) && (flags & (ImGuiWindowFlags_ChildWindow |
        ImGuiWindowFlags_Popup))) window->RootWindowForTitleBarHighlight = parent_window->
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if ((flags & ImGuiWindowFlags_NoInputs) == ImGuiWindowFlags_NoInputs) flags |= ImGuiWindowFlags_NoMove |
    ...
    if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasDock) SetWindowDock(
        window,
    ...
    if (window_just_appearing_after_hidden_for_resize && !(flags & ImGuiWindowFlags_ChildWindow)) window->NavLastIds[0]
        = 0;
    ...
    if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasContentSize) window->ContentSizeExplicit = g.NextWindowData
        .ContentSizeVal;
    else if (first_begin_of_the_frame) window->ContentSizeExplicit = ImVec2(0.0f, 0.0f);
    if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasWindowClass) window->WindowClass = g.NextWindowData.
        WindowClass;
    if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasCollapsed) SetWindowCollapsed(
        window,
    ...
    if (first_begin_of_the_frame)
    {
        ...
        if (window_just_created && (!window_size_x_set_by_api || !window_size_y_set_by_api)) window->
            HiddenFramesCannotSkipItems = 1;
        ...
        else window->WindowBorderSize = ((flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_Tooltip)) && !(flags &
        ...
        else window->WindowPadding = style.WindowPadding;
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar) && !(flags & ImGuiWindowFlags_NoCollapse) && !window->DockIsActive)
        {
            ...
            if (g.HoveredWindow == window && g.HoveredId == 0 && g.HoveredIdPreviousFrame == 0 &&
                IsMouseHoveringRect(title_bar_rect.Min, title_bar_rect.Max) && g.IO.MouseDoubleClicked[0]) window->
                WantCollapseToggle = true;
        ...
        if (flags & ImGuiWindowFlags_ChildWindow)
        {
            ...
            if (!(flags & ImGuiWindowFlags_Popup) && !window_pos_set_by_api && !window_is_child_tooltip) window->Pos =
        ...
        const bool window_pos_with_pivot = (window->SetWindowPosVal.x != FLT_MAX && window->HiddenFramesCannotSkipItems
            == 0);
        if (window_pos_with_pivot) SetWindowPos(window,
            ...
            window_just_appearing_after_hidden_for_resize) window->Pos = FindBestWindowPosForPopup(window);
        else if ((flags & ImGuiWindowFlags_Tooltip) != 0 && !window_pos_set_by_api && !window_is_child_tooltip) window->
        ...
        if (window->ViewportOwned)
        {
            ...
            const ImGuiViewportFlags viewport_flags_to_clear = ImGuiViewportFlags_TopMost |
                ImGuiViewportFlags_NoTaskBarIcon | ImGuiViewportFlags_NoDecoration | ImGuiViewportFlags_NoRendererClear;
            ImGuiViewportFlags viewport_flags         = window->Viewport->Flags & ~viewport_flags_to_clear;
            const bool is_short_lived_floating_window = (flags & (ImGuiWindowFlags_ChildMenu | ImGuiWindowFlags_Tooltip
                | ImGuiWindowFlags_Popup)) != 0;
            if (flags & ImGuiWindowFlags_Tooltip) viewport_flags |= ImGuiViewportFlags_TopMost;
            if (g.IO.ConfigViewportsNoTaskBarIcon || is_short_lived_floating_window) viewport_flags |=
                ImGuiViewportFlags_NoTaskBarIcon;
            if (g.IO.ConfigViewportsNoDecoration || is_short_lived_floating_window) viewport_flags |=
            ...
            if (is_short_lived_floating_window && (flags & ImGuiWindowFlags_Modal) == 0) viewport_flags |=
            ...
            if (window->WindowClass.ParentViewportId) window->Viewport->ParentViewportId = window->WindowClass.
                ParentViewportId;
            else if ((flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_Tooltip)) && parent_window_in_stack) window->
                Viewport->ParentViewportId          = parent_window_in_stack->Viewport->ID;
            else window->Viewport->ParentViewportId = g.IO.ConfigViewportsNoDefaultParent
                                                          ? 0
                                                          : IMGUI_VIEWPORT_DEFAULT_ID;
            if (window->WindowClass.ViewportFlagsOverrideSet) viewport_flags |= window->WindowClass.
                ViewportFlagsOverrideSet;
            if (window->WindowClass.ViewportFlagsOverrideClear) viewport_flags &= ~window->WindowClass.
        ...
        if (window->ViewportOwned || window->DockIsActive) window->WindowRounding = 0.0f;
        else window->WindowRounding = (flags & ImGuiWindowFlags_ChildWindow)
                                          ? style.ChildRounding
        ...
        if (window_just_activated_by_user && !(flags & ImGuiWindowFlags_NoFocusOnAppearing))
        {
            if (flags & ImGuiWindowFlags_Popup) want_focus = true;
                ...
                ImGuiWindowFlags_Tooltip)) want_focus = true;
        }
        ...
        if (handle_borders_and_resize_grips && !window->Collapsed)
            if (UpdateWindowManualResize(window,
                                         ...
                                         resize_grip_count,
                                         &resize_grip_col[0],
                                         visibility_rect)) use_current_size_for_scrollbar_x =
                use_current_size_for_scrollbar_y                                            = true;
        ...
        if (!window->Collapsed)
        {
            ...
            if (window->ScrollbarX && !window->ScrollbarY) window->ScrollbarY = (needed_size_from_last_frame.y >
                size_y_for_scrollbars) && !(flags & ImGuiWindowFlags_NoScrollbar);
        ...
        window->ScrollMax.x = ImMax(0.0f,
                                    window->ContentSize.x + window->WindowPadding.x * 2.0f - window->InnerRect.
                                    GetWidth());
        window->ScrollMax.y = ImMax(0.0f,
                                    window->ContentSize.y + window->WindowPadding.y * 2.0f - window->InnerRect.
                                    GetHeight());
        ...
        // Title bar
        if (!(flags & ImGuiWindowFlags_NoTitleBar) && !window->DockIsActive) RenderWindowTitleBarContents(
            window,
            title_bar_rect,
            name,
            p_open);
        ...
        */
        ...
        if (window->DockIsActive) SetLastItemData(window,
                                                  window->ID,
                                                  window->DockTabItemStatusFlags,
                                                  window->DockTabItemRect);
        else SetLastItemData(window,
                             window->MoveId,
    ...
    if (!(flags & ImGuiWindowFlags_DockNodeHost)) PushClipRect(window->InnerClipRect.Min,
    ...
    if (flags & ImGuiWindowFlags_ChildWindow)
    {
        ...
        if (!(flags & ImGuiWindowFlags_AlwaysAutoResize) && window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0)
            if (window->OuterRectClipped.Min.x >= window->OuterRectClipped.Max.x || window->OuterRectClipped.Min.y >=
                window->OuterRectClipped.Max.y) window->HiddenFramesCanSkipItems = 1;

        // Hide along with parent or if parent is collapsed
        if (parent_window && (parent_window->Collapsed || parent_window->HiddenFramesCanSkipItems > 0)) window->
            HiddenFramesCanSkipItems = 1;
        if (parent_window && (parent_window->Collapsed || parent_window->HiddenFramesCannotSkipItems > 0)) window->
            HiddenFramesCannotSkipItems = 1;
    }
...
void ImGui::FocusWindow(ImGuiWindow* window)
{
    ...
    // Select in dock node
    if (dock_node && dock_node->TabBar) dock_node->TabBar->SelectedTabId = dock_node->TabBar->NextSelectedTabId = window
        ->ID;

    // Bring to front
    BringWindowToFocusFront(focus_front_window);
    if (((window->Flags | focus_front_window->Flags | display_front_window->Flags) &
        ImGuiWindowFlags_NoBringToFrontOnFocus) == 0) BringWindowToDisplayFront(display_front_window);
}
...
{
    ImGuiWindow* window = GetCurrentWindow();
    if (enabled) window->DC.ItemFlags |= option;
    else window->DC.ItemFlags &= ~option;
...
bool ImGui::IsWindowHovered(ImGuiHoveredFlags flags)
{
    ...
    if (flags & ImGuiHoveredFlags_AnyWindow) { if (g.HoveredWindow == nullptr) return false; }
    else
        switch (flags & (ImGuiHoveredFlags_RootWindow | ImGuiHoveredFlags_ChildWindows))
        {
            case ImGuiHoveredFlags_RootWindow | ImGuiHoveredFlags_ChildWindows: if (g.HoveredWindow == nullptr || g.
                    HoveredWindow->RootWindowDockStop != g.CurrentWindow->RootWindowDockStop) return false;
                break;
            case ImGuiHoveredFlags_RootWindow: if (g.HoveredWindow != g.CurrentWindow->RootWindowDockStop) return false;
                break;
            case ImGuiHoveredFlags_ChildWindows: if (g.HoveredWindow == nullptr || !IsWindowChildOf(
                    g.HoveredWindow,
                    g.CurrentWindow)) return false;
                break;
            default: if (g.HoveredWindow != g.CurrentWindow) return false;
                break;
        }
...
bool ImGui::IsWindowFocused(ImGuiFocusedFlags flags)
{
    ...
    if (flags & ImGuiFocusedFlags_AnyWindow) return g.NavWindow != nullptr;

    IM_ASSERT(g.CurrentWindow);     // Not inside a Begin()/End()
    switch (flags & (ImGuiFocusedFlags_RootWindow | ImGuiFocusedFlags_ChildWindows))
    {
        case ImGuiFocusedFlags_RootWindow | ImGuiFocusedFlags_ChildWindows: return g.NavWindow && g.NavWindow->
                RootWindowDockStop == g.CurrentWindow->RootWindowDockStop;
...
{
    if (name) { if (ImGuiWindow* window = FindWindowByName(name)) FocusWindow(window); }
...
static void ImGui::ErrorCheckNewFrameSanityChecks()
{
    ...
    if (g.IO.ConfigWindowsResizeFromEdges && !(g.IO.BackendFlags & ImGuiBackendFlags_HasMouseCursors)) g.IO.
    ...
    if (g.IO.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
    {
            ...
            ImGuiBackendFlags_RendererHasViewports))
        {
            ...
            #ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
            IM_ASSERT(
                g.IO.RenderDrawListsFn == NULL);  // Call ImGui::Render() then pass ImGui::GetDrawData() yourself to your render function!
            #endif
        }
        else
...
static void ImGui::ErrorCheckBeginEndCompareStacksSize(ImGuiWindow* window, bool write)
{
    ...
    // NOT checking: DC.ItemWidth, DC.AllowKeyboardFocus, DC.ButtonRepeat, DC.TextWrapPos (per window) to allow user to conveniently push once and not pop (they are cleared on Begin)
    {
        ...
        if (write) *p = static_cast<short>(n);
        else
...
void ImGui::PushMultiItemsWidths(int components, float w_full)
{
    ...
    const float w_item_one  = ImMax(1.0f,
                                    IM_FLOOR(
                                        (w_full - (style.ItemInnerSpacing.x) * (components - 1)) / static_cast<float>(
                                            components)));
...
void ImGui::EndGroup()
{
    ...
    else if (group_contains_prev_active_id) window->DC.LastItemId = g.ActiveIdPreviousFrame;
    window->DC.LastItemRect = group_bb;

    // Forward Edited flag
    if (group_contains_curr_active_id && g.ActiveIdHasBeenEditedThisFrame) window->DC.LastItemStatusFlags |=
        ImGuiItemStatusFlags_Edited;

    // Forward Deactivated flag
    window->DC.LastItemStatusFlags |= ImGuiItemStatusFlags_HasDeactivated;
    if (group_contains_prev_active_id && g.ActiveId != g.ActiveIdPreviousFrame) window->DC.LastItemStatusFlags |=
...
ImVec2 ImGui::ScrollToBringRectIntoView(ImGuiWindow* window, const ImRect& item_rect)
{
    ...
    {
        if (window->ScrollbarX && item_rect.Min.x < window_rect.Min.x) SetScrollFromPosX(
            window,
            item_rect.Min.x - window->Pos.x - g.Style.ItemSpacing.x,
            0.0f);
        else if (window->ScrollbarX && item_rect.Max.x >= window_rect.Max.x) SetScrollFromPosX(
            window,
            item_rect.Max.x - window->Pos.x + g.Style.ItemSpacing.x,
            1.0f);
        if (item_rect.Min.y < window_rect.Min.y) SetScrollFromPosY(window,
                                                                   item_rect.Min.y - window->Pos.y - g.Style.ItemSpacing
                                                                   .y,
                                                                   0.0f);
        else if (item_rect.Max.y >= window_rect.Max.y) SetScrollFromPosY(
            window,
    ...
    if (window->Flags & ImGuiWindowFlags_ChildWindow) delta_scroll += ScrollToBringRectIntoView(
        window->ParentWindow,
...
void ImGui::SetScrollFromPosY(ImGuiWindow* window, float local_y, float center_y_ratio)
{
    ...
    local_y -= window->TitleBarHeight() + window->
...
bool ImGui::IsPopupOpen(const char* str_id, ImGuiPopupFlags popup_flags)
{
    ...
    if ((popup_flags & ImGuiPopupFlags_AnyPopupLevel) && id != 0)
        IM_ASSERT(
        0 && "Cannot use IsPopupOpen() with a string id and ImGuiPopupFlags_AnyPopupLevel."); // But non-string version is legal and used internally
...
void ImGui::ClosePopupToLevel(int remaining, bool restore_focus_to_window_under_popup)
{
    ...
    if (restore_focus_to_window_under_popup)
    {
        ...
        {
            if (g.NavLayer == ImGuiNavLayer_Main && focus_window) focus_window = NavRestoreLastChildNavWindow(
                focus_window);
            FocusWindow(focus_window);
        }
    }
...
void ImGui::CloseCurrentPopup()
{
    ...
    while (popup_idx > 0)
    {
        ...
        if (popup_window && (popup_window->Flags & ImGuiWindowFlags_ChildMenu))
            if (parent_popup_window == nullptr || !(parent_popup_window->Flags & ImGuiWindowFlags_Modal)) close_parent =
                true;
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags flags)
{
    ...
    char name[20];
    if (flags & ImGuiWindowFlags_ChildMenu) ImFormatString(name,
                                                           IM_ARRAYSIZE(name),
                                                           "##Menu_%02d",
                                                           g.BeginPopupStack.Size); // Recycle windows based on depth
    else ImFormatString(name,
                        IM_ARRAYSIZE(name),
                        "##Popup_%08x",
...
bool ImGui::BeginPopupModal(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if ((g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasPos) == 0)
    {
        ImGuiViewportP* viewport = window->WasActive
                                       ? window->Viewport
                                       : static_cast<ImGuiViewportP*>(GetMainViewport()
...
bool ImGui::BeginPopupContextItem(const char* str_id, ImGuiPopupFlags popup_flags)
{
    ...
    IM_ASSERT(id != 0);                                                  // You cannot pass a NULL str_id if the last item has no identifier (e.g. a Text() item)
    int mouse_button = (popup_flags & ImGuiPopupFlags_MouseButtonMask_);
    if (IsMouseReleased(mouse_button) && IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup)) OpenPopupEx(
        id,
        popup_flags);
...
ImVec2 ImGui::FindBestWindowPosForPopup(ImGuiWindow* window)
{
    ...
    if (window->Flags & ImGuiWindowFlags_ChildMenu)
    {
        ...
        float horizontal_overlap   = g.Style.ItemInnerSpacing.
        ...
        ImRect r_avoid;
        if (parent_window->DC.MenuBarAppending) r_avoid =
            ImRect(-FLT_MAX,
        ...
        else r_avoid = ImRect(parent_window->Pos.x + horizontal_overlap,
                              -FLT_MAX,
    ...
    if (window->Flags & ImGuiWindowFlags_Tooltip)
    {
        ...
        if (!g.NavDisableHighlight && g.NavDisableMouseHover && !(g.IO.ConfigFlags &
            ImGuiConfigFlags_NavEnableSetMousePos)) r_avoid = ImRect(ref_pos.x - 16,
                                                                     ref_pos.y - 8,
                                                                     ref_pos.x + 16,
                                                                     ref_pos.y + 8);
        else r_avoid =
        ...
        if (window->AutoPosLastDirection == ImGuiDir_None) pos = ref_pos +
            ImVec2(2,
...
void ImGui::SetFocusID(ImGuiID id, ImGuiWindow* window)
{
    ...
    if (window->DC.LastItemId == id) window->NavRectRel[nav_layer] = ImRect(
...
static bool ImGui::NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    const ImRect& curr = g.
        NavScoringRect; // Current modified source rect (NB: we've applied Max.x = Min.x in NavUpdate() to inhibit the effect of having varied item width)
    g.NavScoringCount++;
...
static void ImGui::NavSaveLastChildNavWindowIntoParent(ImGuiWindow* nav_window)
{
    ...
    while (parent && parent->RootWindowDockStop != parent && (parent->Flags & ImGuiWindowFlags_ChildWindow) != 0 && (
        parent->Flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu)) == 0) parent = parent->ParentWindow;
...
static ImGuiWindow* ImGui::NavRestoreLastChildNavWindow(ImGuiWindow* window)
{
    ...
    if (window->DockNodeAsHost && window->DockNodeAsHost->TabBar)
        if (ImGuiTabItem* tab = TabBarFindMostRecentlySelectedTabForActiveWindow(window->DockNodeAsHost->TabBar)) return
            tab->Window;
    return window;
}
...
static void NavRestoreLayer(ImGuiNavLayer layer)
{
    ...
    if (window->NavLastIds[layer] != 0) ImGui::SetNavIDWithRectRel(window->NavLastIds[layer],
                                                                   layer,
                                                                   0,
...
void ImGui::NavInitWindow(ImGuiWindow* window, bool force_reinit)
{
    ...
    if (!(window->Flags & ImGuiWindowFlags_NoNavInputs))
        if (!(window->Flags & ImGuiWindowFlags_ChildWindow) || (window->Flags & ImGuiWindowFlags_Popup) || (window->
            NavLastIds[0] == 0) || force_reinit) init_for_nav = true;
...
{
    ImGuiContext& g = *GImGui;
    if (mode == ImGuiInputReadMode_Down) return g.IO.NavInputs[n
    ...
    if (mode == ImGuiInputReadMode_Repeat)
        return static_cast<float>(CalcTypematicRepeatAmount(t - g.IO.DeltaTime,
                                                            t,
                                  ...
                                  float fast_factor)
{
    ...
    if (dir_sources & ImGuiNavDirSourceFlags_Keyboard) delta += ImVec2(
    ...
    if (dir_sources & ImGuiNavDirSourceFlags_PadDPad) delta += ImVec2(
    ...
    if (dir_sources & ImGuiNavDirSourceFlags_PadLStick) delta += ImVec2(
...
static void ImGui::NavUpdate()
{
    ...
    if (nav_gamepad_active)
        if (g.IO.NavInputs[ImGuiNavInput_Activate] > 0.0f || g.IO.NavInputs[ImGuiNavInput_Input] > 0.0f || g.IO.
            NavInputs[ImGuiNavInput_Cancel] > 0.0f || g.IO.NavInputs[ImGuiNavInput_Menu] > 0.0f) g.NavInputSource =
    ...
    for (int i = 0; i < IM_ARRAYSIZE(g.IO.NavInputs); i++) g.IO.NavInputsDownDuration[i] = (g.IO.NavInputs[i] > 0.0f)
    ...
    if (g.NavInitResultId != 0 && (!g.NavDisableHighlight || g.NavInitRequestFromMove) && g.NavWindow)
    {
        ...
        if (g.NavInitRequestFromMove) SetNavIDWithRectRel(g.NavInitResultId, g.NavLayer, 0, g.NavInitResultRectRel);
    ...
    if (g.NavWindow && g.NavWindow->NavLastChildNavWindow != nullptr && g.NavLayer == ImGuiNavLayer_Main) g.NavWindow->
        NavLastChildNavWindow = nullptr;
    ...
    {
        if (g.ActiveId != 0) { if (!IsActiveIdUsingNavInput(ImGuiNavInput_Cancel)) ClearActiveID(); }
        else if (g.NavWindow && (g.NavWindow->Flags & ImGuiWindowFlags_ChildWindow) && !(g.NavWindow->Flags &
            ImGuiWindowFlags_Popup) && g.NavWindow->ParentWindow && g.NavWindow != g.NavWindow->RootWindowDockStop)
        {
            // Exit child window
        ...
        else if (g.OpenPopupStack.Size > 0)
        {
            ...
            if (!(g.OpenPopupStack.back().Window->Flags & ImGuiWindowFlags_Modal)) ClosePopupToLevel(
        ...
        else
        {
            ...
            if (g.NavWindow && ((g.NavWindow->Flags & ImGuiWindowFlags_Popup) || !(g.NavWindow->Flags &
                ImGuiWindowFlags_ChildWindow))) g.NavWindow->NavLastIds[0] = 0;
        ...
        ImGuiWindowFlags_NoNavInputs))
    {
        ...
        if ((g.ActiveId == 0 || g.ActiveId == g.NavId) && IsNavInputTest(
            ImGuiNavInput_Input,
            ImGuiInputReadMode_Pressed)) g.NavInputId = g.NavId;
    }
    ...
    if (g.NavNextActivateId != 0) g.NavActivateId = g.NavActivateDownId = g.NavActivatePressedId = g.NavInputId = g.
        NavNextActivateId;
    ...
    if (g.NavWindow && !(g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) && !g.NavWindowingTarget)
    {
        ...
        const float scroll_speed =
            IM_ROUND(
                window->CalcFontSize() * 100 * g.IO.
                DeltaTime); // We need round the scrolling speed because sub-pixel scroll isn't reliably supported.
        if (window->DC.NavLayerActiveMask == 0x00 && window->DC.NavHasScroll && g.NavMoveRequest)
        {
            if (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right) SetScrollX(
                window,
                ImFloor(window->Scroll.x + ((g.NavMoveDir == ImGuiDir_Left) ? -1.0f : +1.0f) * scroll_speed));
            if (g.NavMoveDir == ImGuiDir_Up || g.NavMoveDir == ImGuiDir_Down) SetScrollY(
                window,
...
static void ImGui::NavUpdateMoveResult()
{
    ...
    if (g.NavMoveRequestFlags & ImGuiNavMoveFlags_AlsoScoreVisibleSet)
        if (g.NavMoveResultLocalVisibleSet.ID != 0 && g.NavMoveResultLocalVisibleSet.ID != g.NavId) result = &g.
    ...
    if (result != &g.NavMoveResultOther && g.NavMoveResultOther.ID != 0 && g.NavMoveResultOther.Window->ParentWindow ==
        ...
        if ((g.NavMoveResultOther.DistBox < result->DistBox) || (g.NavMoveResultOther.DistBox == result->DistBox && g.
            NavMoveResultOther.DistCenter < result->DistCenter)) result = &g.NavMoveResultOther;
...
{
    ImGuiContext& g = *GImGui;
    if (g.NavMoveDir != ImGuiDir_None || g.NavWindow == nullptr) return 0.0f;
    if ((g.NavWindow->Flags & ImGuiWindowFlags_NoNavInputs) || g.NavWindowingTarget != nullptr || g.NavLayer !=
        ImGuiNavLayer_Main) return 0.0f;
    ...
    if (page_up_held != page_down_held || home_pressed != end_pressed) // If either (not both) are pressed
    {
        if (window->DC.NavLayerActiveMask == 0x00 && window->DC.NavHasScroll)
        {
            ...
            if (IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageUp], true)) SetScrollY(
                window,
                window->Scroll.y - window->InnerRect.GetHeight());
            else if (IsKeyPressed(g.IO.KeyMap[ImGuiKey_PageDown], true)) SetScrollY(
                window,
                window->Scroll.y + window->InnerRect.GetHeight());
        ...
        ImGuiNavForward_None && g.NavLayer == ImGuiNavLayer_Main)
    {
        ...
        if (g.NavMoveDir == ImGuiDir_Left && (move_flags & (ImGuiNavMoveFlags_WrapX | ImGuiNavMoveFlags_LoopX)))
        {
            bb_rel.Min.x = bb_rel.Max.x = ImMax(window->SizeFull.x,
                                                window->ContentSize.x + window->WindowPadding.x * 2.0f) - window->Scroll
                .x;
        ...
        if (g.NavMoveDir == ImGuiDir_Up && (move_flags & (ImGuiNavMoveFlags_WrapY | ImGuiNavMoveFlags_LoopY)))
        {
            bb_rel.Min.y = bb_rel.Max.y = ImMax(window->SizeFull.y,
                                                window->ContentSize.y + window->WindowPadding.y * 2.0f) - window->Scroll
                .y;
...
static void NavUpdateWindowingHighlightWindow(int focus_change_dir)
{
    ...
    if (!window_target) window_target = FindWindowNavFocusable(
...
static void ImGui::NavUpdateWindowing()
{
    ...
    {
        ImVec2 move_delta;
        if (g.NavInputSource == ImGuiInputSource_NavKeyboard && !g.IO.KeyShift) move_delta = GetNavInputAmount2d(
            ImGuiNavDirSourceFlags_Keyboard,
            ImGuiInputReadMode_Down);
        if (g.NavInputSource == ImGuiInputSource_NavGamepad) move_delta = GetNavInputAmount2d(
    ...
    if (apply_focus_window && (g.NavWindow == nullptr || apply_focus_window != g.NavWindow->RootWindowDockStop))
    {
        ...
        if (apply_focus_window->Viewport != previous_viewport && g.PlatformIO.Platform_SetWindowFocus) g.PlatformIO.
            Platform_SetWindowFocus(apply_focus_window->Viewport);
    }
    ...
    if (apply_toggle_layer && g.NavWindow)
    {
        ...
        while (new_nav_window->ParentWindow && (new_nav_window->DC.NavLayerActiveMask & (1 << ImGuiNavLayer_Menu)) == 0
            && (new_nav_window->Flags & ImGuiWindowFlags_ChildWindow) != 0 && (new_nav_window->Flags & (
                ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu)) == 0) new_nav_window = new_nav_window->
            ParentWindow;
        ...
        if (new_nav_layer == ImGuiNavLayer_Menu && !preserve_layer_1_nav_id) g.NavWindow->NavLastIds[ImGuiNavLayer_Menu]
            = 0;
...
static const char* GetFallbackWindowNameForWindowingList(ImGuiWindow* window)
{
    ...
    if ((window->Flags & ImGuiWindowFlags_MenuBar) && strcmp(window->Name, "##MainMenuBar") == 0) return
        "(Main menu bar)";
...
void ImGui::NavUpdateWindowingOverlay()
{
    ...
    ImGuiViewportP* viewport = /*g.NavWindow ? g.NavWindow->Viewport :*/ static_cast<ImGuiViewportP*>(GetMainViewport()
    );
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags)
{
    ...
    if (!(flags & ImGuiDragDropFlags_SourceExtern))
    {
        ...
        if (source_id == 0)
        {
            ...
            if ((window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_HoveredRect) == 0 && (g.ActiveId == 0 || g.
                ActiveIdWindow != window)) return false;
    ...
    if (source_drag_active)
    {
        ...
        if (!(flags & ImGuiDragDropFlags_SourceNoDisableHover) && !(flags & ImGuiDragDropFlags_SourceExtern)) window->DC
            .LastItemStatusFlags &= ~ImGuiItemStatusFlags_HoveredRect;
...
void ImGui::LogRenderedText(const ImVec2* ref_pos, const char* text, const char* text_end)
{
    ...
    for (;;)
    {
        ...
        if (!is_last_line || (line_start != line_end))
        {
            ...
            if (log_new_line || !is_first_line) LogText(IM_NEWLINE "%*s%.*s",
                                                        tree_depth * 4,
                                                        "",
                                                        char_count,
                                                        line_start);
            else if (g.LogLineFirstItem) LogText("%*s%.*s", tree_depth * 4, "", char_count, line_start);
...
void ImGui::LogFinish()
{
    ...
    switch (g.LogType)
    {
        ...
        case ImGuiLogType_Clipboard: if (!g.LogBuffer.empty()) SetClipboardText(g.LogBuffer.begin());
            break;
...
void ImGui::ClearIniSettings()
{
    ...
    for (int handler_n = 0; handler_n < g.SettingsHandlers.Size; handler_n++)
        if (g.SettingsHandlers[handler_n].ClearAllFn) g.SettingsHandlers[handler_n].ClearAllFn(
            &g,
...
void ImGui::LoadIniSettingsFromMemory(const char* ini_data, size_t ini_size)
{
    ...
    for (int handler_n = 0; handler_n < g.SettingsHandlers.Size; handler_n++)
        if (g.SettingsHandlers[handler_n].ReadInitFn) g.SettingsHandlers[handler_n].ReadInitFn(
            &g,
            &g.SettingsHandlers[handler_n]);
    ...
    for (int handler_n = 0; handler_n < g.SettingsHandlers.Size; handler_n++)
        if (g.SettingsHandlers[handler_n].ApplyAllFn) g.SettingsHandlers[handler_n].ApplyAllFn(
            &g,
            &g.SettingsHandlers[handler_n]);
}
...
static void WindowSettingsHandler_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line)
{
    ...
    else if (sscanf(line, "Size=%i,%i", &x, &y) == 2) settings->Size = ImVec2ih(
        static_cast<short>(x),
        static_cast<short>(y));
    else if (sscanf(line, "ViewportId=0x%08X", &u1) == 1) settings->ViewportId = u1;
    else if (sscanf(line, "ViewportPos=%i,%i", &x, &y) == 2) settings->ViewportPos = ImVec2ih(
        static_cast<short>(x),
        static_cast<short>(y));
...
static void WindowSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
         ...
         next_chunk(settings))
    {
        ...
        if (settings->Size.x != 0 || settings->Size.y != 0) buf->appendf(
            "Size=%d,%d\n",
            settings->Size.x,
            settings->Size.y);
        ...
        if (settings->DockId != 0)
        {
            ...
            if (settings->DockOrder == -1) buf->appendf("DockId=0x%08X\n", settings->DockId);
            else buf->appendf("DockId=0x%08X,%d\n", settings->DockId, settings->DockOrder);
...
void ImGui::SetCurrentViewport(ImGuiWindow* current_window, ImGuiViewportP* viewport)
{
    ...
    if (g.CurrentViewport && g.PlatformIO.Platform_OnChangedViewport) g.PlatformIO.Platform_OnChangedViewport(
        g.CurrentViewport);
}
...
void ImGui::TranslateWindowsInViewport(ImGuiViewportP* viewport, const ImVec2& old_pos, const ImVec2& new_pos)
{
    ...
    for (int window_n = 0; window_n < g.Windows.Size; window_n++) // FIXME-OPT
        if (translate_all_windows || (g.Windows[window_n]->Viewport == viewport && test_still_fit_rect.
            Contains(g.Windows[window_n]->Rect()))) TranslateWindow(g.Windows[window_n], delta_pos);
}
...
static void ImGui::UpdateViewportsNewFrame()
{
    ...
    if (viewports_enabled)
        for (int n = 0; n < g.Viewports.Size; n++)
        {
            ...
            if (g.PlatformIO.Platform_GetWindowMinimized && platform_funcs_available)
            {
                ...
                if (minimized) viewport->Flags |= ImGuiViewportFlags_Minimized;
                else viewport->Flags &= ~ImGuiViewportFlags_Minimized;
            }
        }
    ...
    {
        main_viewport_pos = main_viewport->
            Pos;    // Preserve last pos/size when minimized (FIXME: We don't do the same for Size outside of the viewport path)
    ...
    for (int n = 0; n < g.Viewports.Size; n++)
    {
        ...
        if (viewports_enabled)
            ...
            {
                if (viewport->PlatformRequestMove) viewport->Pos = viewport->LastPlatformPos = g.PlatformIO.
                    Platform_GetWindowPos(viewport);
                if (viewport->PlatformRequestResize) viewport->Size = viewport->LastPlatformSize = g.PlatformIO.
        ...
        if ((viewport->Flags & ImGuiViewportFlags_CanHostOtherWindows) && (viewport_delta_pos.x != 0.0f ||
            viewport_delta_pos.y != 0.0f)) TranslateWindowsInViewport(viewport, viewport->LastPos, viewport->Pos);

        // Update DPI scale
        float new_dpi_scale;
        if (g.PlatformIO.Platform_GetWindowDpiScale && platform_funcs_available) new_dpi_scale = g.PlatformIO.
            Platform_GetWindowDpiScale(viewport);
        else if (viewport->PlatformMonitor != -1) new_dpi_scale = g.PlatformIO.Monitors[viewport->PlatformMonitor].
            DpiScale;
        else new_dpi_scale = (viewport->DpiScale != 0.0f) ? viewport->DpiScale : 1.0f;
        if (viewport->DpiScale != 0.0f && new_dpi_scale != viewport->DpiScale)
        {
            float scale_factor = new_dpi_scale / viewport->DpiScale;
            if (g.IO.ConfigFlags & ImGuiConfigFlags_DpiEnableScaleViewports) ScaleWindowsInViewport(
                viewport,
                scale_factor);
    ...
    if (g.IO.BackendFlags & ImGuiBackendFlags_HasMouseHoveredViewport)
    {
        ...
        viewport_hovered = FindHoveredViewportFromPlatformWindowStack(g.IO.MousePos);
    if (viewport_hovered != nullptr) g.MouseLastHoveredViewport = viewport_hovered;
    else if (g.MouseLastHoveredViewport == nullptr) g.MouseLastHoveredViewport = g.Viewports[0];

    // Update mouse reference viewport
    // (when moving a window we aim at its viewport, but this will be overwritten below if we go in drag and drop mode)
    if (g.MovingWindow) g.MouseViewport = g.MovingWindow->Viewport;
    ...
    if (is_mouse_dragging_with_an_expected_destination && viewport_hovered == nullptr) viewport_hovered = g.
        MouseLastHoveredViewport;
    if (is_mouse_dragging_with_an_expected_destination || g.ActiveId == 0 || !IsAnyMouseDown())
        if (viewport_hovered != nullptr && viewport_hovered != g.MouseViewport && !(viewport_hovered->Flags &
            ImGuiViewportFlags_NoInputs)) g.MouseViewport = viewport_hovered;
                                         ...
                                         ImGuiViewportFlags flags)
{
    ...
    if (window != nullptr)
    {
        if (g.MovingWindow && g.MovingWindow->RootWindow == window) flags |= ImGuiViewportFlags_NoInputs |
            ImGuiViewportFlags_NoFocusOnAppearing;
        if ((window->Flags & ImGuiWindowFlags_NoMouseInputs) && (window->Flags & ImGuiWindowFlags_NoNavInputs)) flags |=
    ...
    else
    {
        ...
        if (viewport->PlatformMonitor != -1) viewport->DpiScale = g.PlatformIO.Monitors[viewport->PlatformMonitor].
            DpiScale;
    }
...
static void ImGui::UpdateSelectWindowViewport(ImGuiWindow* window)
{
    ...
    if ((g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasViewport) == 0)
    {
        ...
        if (window->Viewport == nullptr && window->ParentWindow && !window->ParentWindow->IsFallbackWindow) window->
            Viewport = window->ParentWindow->Viewport;
    ...
    else if (GetWindowAlwaysWantOwnViewport(window)) window->Viewport = AddUpdateViewport(
        window,
        window->ID,
        window->Pos,
        window->Size,
    ...
    {
        if (window->Viewport != nullptr && window->Viewport->Window == window) window->Viewport = AddUpdateViewport(
            window,
            window->ID,
            window->Pos,
            window->Size,
    ...
    if (window->Viewport == nullptr) window->Viewport = main_viewport;
    ...
    if (!lock_viewport)
    {
        if (flags & (ImGuiWindowFlags_Tooltip | ImGuiWindowFlags_Popup))
        {
            ...
            if ((window->Appearing || (flags & (ImGuiWindowFlags_Tooltip | ImGuiWindowFlags_ChildMenu))) && (!
                use_mouse_ref || mouse_valid)) window->ViewportAllowPlatformMonitorExtend = FindPlatformMonitorForPos(
...
void ImGui::UpdatePlatformWindows()
{
    ...
    for (int i = 1; i < g.Viewports.Size; i++)
    {
        ...
        if (is_new_platform_window)
        {
            ...
            viewport->LastRendererSize = viewport->
                Size;                                       // We don't need to call Renderer_SetWindowSize() as it is expected Renderer_CreateWindow() already did it.
            viewport->PlatformWindowCreated = true;
        }
        ...
        if ((viewport->LastPlatformPos.x != viewport->Pos.x || viewport->LastPlatformPos.y != viewport->Pos.y) && !
            viewport->PlatformRequestMove) g.PlatformIO.Platform_SetWindowPos(viewport, viewport->Pos);
        if ((viewport->LastPlatformSize.x != viewport->Size.x || viewport->LastPlatformSize.y != viewport->Size.y) && !
            viewport->PlatformRequestResize) g.PlatformIO.Platform_SetWindowSize(viewport, viewport->Size);
        if ((viewport->LastRendererSize.x != viewport->Size.x || viewport->LastRendererSize.y != viewport->Size.y) && g.
            PlatformIO.Renderer_SetWindowSize) g.PlatformIO.Renderer_SetWindowSize(viewport, viewport->Size);
        ...
        if (viewport->LastAlpha != viewport->Alpha && g.PlatformIO.Platform_SetWindowAlpha) g.PlatformIO.
            Platform_SetWindowAlpha(viewport, viewport->Alpha);
        ...
        if (is_new_platform_window)
        {
            ...
            if (viewport->LastFrontMostStampCount != g.ViewportFrontMostStampCount) viewport->LastFrontMostStampCount =
    ...
    if (g.PlatformIO.Platform_GetWindowFocus != nullptr)
    {
        ...
        {
            if (focused_viewport->LastFrontMostStampCount != g.ViewportFrontMostStampCount) focused_viewport->
                LastFrontMostStampCount = ++g.ViewportFrontMostStampCount;
...
void ImGui::DestroyPlatformWindow(ImGuiViewportP* viewport)
{
    ...
    if (viewport->PlatformWindowCreated)
    {
        ...
        IM_ASSERT(
        viewport->RendererUserData == NULL && viewport->PlatformUserData == NULL && viewport->PlatformHandle ==
        NULL);
...
struct ImGuiDockRequest
{
    ...
    ImGuiWindow*
    DockTargetWindow;   // Destination/Target Window to dock into (may be a loose window or a DockNode, might be NULL in which case DockTargetNode cannot be NULL)
...
struct ImGuiDockPreviewData
{
    ...
    bool
    IsSplitDirExplicit;         // Set when hovered the drop rect (vs. implicit SplitDir==None when hovered the window)
        ...
        : FutureNode(0)
    {
        ...
        SplitRatio    = 0.f;
        for (int n = 0; n < IM_ARRAYSIZE(DropRectsDraw); n++) DropRectsDraw[n] = ImRect(
            +FLT_MAX,
            +FLT_MAX,
            -FLT_MAX,
            -FLT_MAX);
...
void ImGui::DockContextShutdown(ImGuiContext* ctx)
{
    ...
    for (int n = 0; n < dc->Nodes.Data.Size; n++)
        if (ImGuiDockNode* node = static_cast<ImGuiDockNode*>(dc->Nodes.Data[n].val_p)) IM_DELETE(node);
}
...
void ImGui::DockContextUpdateUndocking(ImGuiContext* ctx)
{
    ...
    if (g.IO.ConfigDockingNoSplit)
        for (int n = 0; n < dc->Nodes.Data.Size; n++)
            if (ImGuiDockNode* node = static_cast<ImGuiDockNode*>(dc->Nodes.Data[n].val_p)) if (node->IsRootNode() &&
                node->IsSplitNode()) DockBuilderRemoveNodeChildNodes(node->ID);
    ...
    for (int n = 0; n < dc->Requests.Size; n++)
    {
        ...
        if (req->Type == ImGuiDockRequestType_Undock && req->UndockTargetWindow) DockContextProcessUndockWindow(
            ctx,
            req->UndockTargetWindow);
        else if (req->Type == ImGuiDockRequestType_Undock && req->UndockTargetNode) DockContextProcessUndockNode(
            ctx,
...
void ImGui::DockContextUpdateDocking(ImGuiContext* ctx)
{
    ...
    {
        if (hovered_window->DockNodeAsHost) g.HoveredDockNode = DockNodeTreeFindVisibleNodeByPos(
            hovered_window->DockNodeAsHost,
            g.IO.MousePos);
        else if (hovered_window->RootWindowDockStop->DockNode) g.HoveredDockNode = hovered_window->RootWindowDockStop->
            DockNode;
    }
        ...
        if (settings->ParentWindowId != 0)
            if (ImGuiWindowSettings* window_settings = FindWindowSettings(settings->ParentWindowId))
                if (window_settings->DockId)
                    if (ImGuiDockContextPruneNodeData* data = pool.GetByKey(window_settings->DockId)) data->
                        CountChildNodes++;
    }
        ...
        if (ImGuiID dock_id                         = settings->DockId)
            if (ImGuiDockContextPruneNodeData* data = pool.GetByKey(dock_id))
            {
                ...
                if (ImGuiDockContextPruneNodeData* data_root = (data->RootId == dock_id)
                                                                   ? data
                                                                   : pool.GetByKey(data->RootId)) data_root->
                    CountChildWindows++;
            }
                                                     ...
                                                     int node_settings_count)
{
    ...
    for (int node_n = 0; node_n < node_settings_count; node_n++)
    {
        ...
        if (node->ParentNode && node->ParentNode->ChildNodes[0] == nullptr) node->ParentNode->ChildNodes[0] = node;
        else if (node->ParentNode && node->ParentNode->ChildNodes[1] == nullptr) node->ParentNode->ChildNodes[1] = node;
...
{
    IM_ASSERT(
        (req->Type == ImGuiDockRequestType_Dock && req->DockPayload != NULL) || (req->Type == ImGuiDockRequestType_Split
    ...
    if (payload_window)
    {
        ...
        if (payload_node && payload_node->IsLeafNode()) next_selected_id = payload_node->TabBar->NextSelectedTabId
                                                                               ? payload_node->TabBar->NextSelectedTabId
                                                                               : payload_node->TabBar->SelectedTabId;
        if (payload_node == nullptr) next_selected_id = payload_window->ID;
    }
    ...
    if (node != payload_node)
    {
        ...
        if (node->Windows.Size > 0 && node->TabBar == nullptr)
        {
            ...
            for (int n = 0; n < node->Windows.Size; n++) TabBarAddTab(node->TabBar,
...
void ImGui::DockContextProcessUndockWindow(ImGuiContext* ctx, ImGuiWindow* window, bool clear_persistent_docking_ref)
{
    ...
    IM_UNUSED(ctx);
    if (window->DockNode) DockNodeRemoveWindow(window->DockNode,
                                               window,
...
void ImGui::DockContextProcessUndockNode(ImGuiContext* ctx, ImGuiDockNode* node)
{
    ...
    if (node->IsRootNode() || node->IsCentralNode())
    {
        ...
        for (int n = 0; n < new_node->Windows.Size; n++) UpdateWindowParentAndRootLinks(
            new_node->Windows[n],
            new_node->Windows[n]->Flags,
            nullptr);
        node = new_node;
    }
    else
    {
        ...
        node->ParentNode->AuthorityForViewport =
            ImGuiDataAuthority_Window; // The node that stays in place keeps the viewport, so our newly dragged out node will create a new viewport
...
static void ImGui::DockNodeAddWindow(ImGuiDockNode* node, ImGuiWindow* window, bool add_to_tab_bar)
{
    ...
    if (node->HostWindow == nullptr && node->IsFloatingNode())
    {
        ...
        if (node->AuthorityForViewport == ImGuiDataAuthority_Auto) node->AuthorityForViewport =
    ...
    if (add_to_tab_bar)
    {
        if (node->TabBar == nullptr)
        {
            ...
            for (int n = 0; n < node->Windows.Size - 1; n++) TabBarAddTab(
                node->TabBar,
                ImGuiTabItemFlags_None,
                node->Windows[n]);
        }
    ...
    if (node->HostWindow) UpdateWindowParentAndRootLinks(window,
...
struct ImGuiDockNodeFindInfoResults
{
    ...
    ImGuiDockNodeFindInfoResults()
    {
        CentralNode           = FirstNodeWithWindows = nullptr;
...
static void DockNodeFindInfo(ImGuiDockNode* node, ImGuiDockNodeFindInfoResults* results)
{
    if (node->Windows.Size > 0)
    {
        if (results->FirstNodeWithWindows == nullptr) results->FirstNodeWithWindows = node;
    ...
    if (results->CountNodesWithWindows > 1 && results->CentralNode != nullptr) return;
...
static void ImGui::DockNodeUpdateVisibleFlagAndInactiveChilds(ImGuiDockNode* node)
{
    ...
    // Inherit most flags
    if (node->ParentNode) node->SharedFlags = node->ParentNode->SharedFlags &
    ...
    if (node->WantHiddenTabBarUpdate && node->Windows.Size == 1 && (node_flags & ImGuiDockNodeFlags_AutoHideTabBar) && !
        node->IsHiddenTabBar()) node->WantHiddenTabBarToggle = true;
    ...
    if (node->WantHiddenTabBarToggle && node->VisibleWindow && (node->VisibleWindow->WindowClass.
        ...
        ImGuiDockNodeFlags_HiddenTabBar)) node->WantHiddenTabBarToggle = false;

    // Apply toggles at a single point of the frame (here!)
    if (node->Windows.Size > 1) node->LocalFlags &= ~ImGuiDockNodeFlags_HiddenTabBar;
    else if (node->WantHiddenTabBarToggle) node->LocalFlags ^= ImGuiDockNodeFlags_HiddenTabBar;
...
static void ImGui::DockNodeStartMouseMovingWindow(ImGuiDockNode* node, ImGuiWindow* window)
{
    ...
    g.MovingWindow        =
        window; // If we are docked into a non moveable root window, StartMouseMovingWindow() won't set g.MovingWindow. Override that decision.
...
static void ImGui::DockNodeUpdateForRootNode(ImGuiDockNode* node)
{
    ...
    if (node->LastFocusedNodeId == 0 && results.FirstNodeWithWindows != nullptr) node->LastFocusedNodeId = results.
        FirstNodeWithWindows->ID;
...
static void ImGui::DockNodeUpdate(ImGuiDockNode* node)
{
    ...
    if (node->Windows.Size <= 1 && node->IsFloatingNode() && node->IsLeafNode())
        if (!g.IO.ConfigDockingAlwaysTabBar && (node->Windows.Size == 0 || !node->Windows[0]->WindowClass.
            DockingAlwaysTabBar)) want_to_hide_host_window = true;
    ...
    else
    {
        ...
        node->HasWindowMenuButton = (node->Windows.Size > 0) && (node_flags & ImGuiDockNodeFlags_NoWindowMenuButton) ==
            0;
        for (int window_n = 0; window_n < node->Windows.Size; window_n++)
        {
        ...
        if (node->IsRootNode() && node->IsVisible)
        {
            ...
            // Sync Pos
            if (node->AuthorityForPos == ImGuiDataAuthority_Window && ref_window) SetNextWindowPos(ref_window->Pos);
            else if (node->AuthorityForPos == ImGuiDataAuthority_DockNode) SetNextWindowPos(node->Pos);

            // Sync Size
            if (node->AuthorityForSize == ImGuiDataAuthority_Window && ref_window) SetNextWindowSize(
                ref_window->SizeFull);
            else if (node->AuthorityForSize == ImGuiDataAuthority_DockNode) SetNextWindowSize(node->Size);

            // Sync Collapsed
            if (node->AuthorityForSize == ImGuiDataAuthority_Window && ref_window) SetNextWindowCollapsed(
                ref_window->Collapsed);

            // Sync Viewport
            if (node->AuthorityForViewport == ImGuiDataAuthority_Window && ref_window) SetNextWindowViewport(
                ref_window->ViewportId);
    ...
    if (node->IsRootNode())
        if (g.NavWindow && g.NavWindow->RootWindowDockStop->DockNode && g.NavWindow->RootWindowDockStop->ParentWindow ==
            host_window) node->LastFocusedNodeId = g.NavWindow->RootWindowDockStop->DockNode->ID;
    ...
    if (central_node_hole)
        if (const ImGuiPayload* payload = GetDragDropPayload())
            if (payload->IsDataType(IMGUI_PAYLOAD_TYPE_WINDOW) && DockNodeIsDropAllowed(
                host_window,
                *static_cast<ImGuiWindow**>(payload->Data))) central_node_hole_register_hit_test_hole = false;
    ...
    if (render_dockspace_bg && node->IsVisible)
    {
        ...
        if (central_node_hole) RenderRectFilledWithHole(host_window->DrawList,
                                                        node->Rect(),
                                                        central_node->Rect(),
                                                        GetColorU32(ImGuiCol_WindowBg),
                                                        0.0f);
        else host_window->DrawList->AddRectFilled(node->Pos,
                                                  node->Pos + node->Size,
                                                  GetColorU32(ImGuiCol_WindowBg),
                                                  0.0f);
    ...
    if (host_window && node->Windows.Size > 0) DockNodeUpdateTabBar(node, host_window);
    else
    {
        node->WantCloseAll   = false;
        node->WantCloseTabId = 0;
    ...
    else if (node->Windows.Size > 0) node->SelectedTabId = node->Windows[0]->ID;
...
static ImGuiID ImGui::DockNodeUpdateWindowMenu(ImGuiDockNode* node, ImGuiTabBar* tab_bar)
{
    ...
    if (g.Style.WindowMenuButtonPosition == ImGuiDir_Left) SetNextWindowPos(
        ImVec2(node->Pos.x, node->Pos.y + GetFrameHeight()),
        ImGuiCond_Always,
        ImVec2(0.0f, 0.0f));
    else SetNextWindowPos(ImVec2(node->Pos.x + node->Size.x, node->Pos.y + GetFrameHeight()),
                          ImGuiCond_Always,
                          ImVec2(1.0f, 0.0f));
...
static void ImGui::DockNodeUpdateTabBar(ImGuiDockNode* node, ImGuiWindow* host_window)
{
    ...
    ImGuiDockNode* root_node = DockNodeGetRootNode(node);
    if (g.NavWindowingTarget) is_focused = (g.NavWindowingTarget->DockNode == node);
    else if (g.NavWindow && g.NavWindow->RootWindowForTitleBarHighlight == host_window->RootWindow && root_node->
        LastFocusedNodeId == node->ID) is_focused = true;
    ...
    {
        if (ImGuiID tab_id = DockNodeUpdateWindowMenu(node, tab_bar)) focus_tab_id = tab_bar->NextSelectedTabId =
            tab_id;
    ...
    if (tab_bar_is_recreated && TabBarFindTabByID(tab_bar, node->SelectedTabId) != nullptr) tab_bar->SelectedTabId =
        tab_bar->NextSelectedTabId = node->SelectedTabId;
    else if (tab_bar->Tabs.Size > tabs_count_old) tab_bar->SelectedTabId = tab_bar->NextSelectedTabId = tab_bar->Tabs.
        back().Window->ID;
    ...
    for (int window_n = 0; window_n < node->Windows.Size; window_n++)
    {
        ...
        if ((closed_all || closed_one == window->ID) && window->HasCloseButton && !(window->Flags &
            ImGuiWindowFlags_UnsavedDocument)) continue;
        if (window->LastFrameActive + 1 >= g.FrameCount || !node_was_active)
        {
            ImGuiTabItemFlags tab_item_flags = 0;
            if (window->Flags & ImGuiWindowFlags_UnsavedDocument) tab_item_flags |= ImGuiTabItemFlags_UnsavedDocument;
            if (tab_bar->Flags & ImGuiTabBarFlags_NoCloseWithMiddleMouseButton) tab_item_flags |=
            ...
            if (g.NavWindow && g.NavWindow->RootWindowDockStop == window && (window->DC.NavLayerActiveMask & (1 <<
                ImGuiNavLayer_Menu)) == 0) host_window->NavLastIds[1] = window->ID;
        }
    }
    ...
    if (g.HoveredId == 0 || g.HoveredId == title_bar_id || g.ActiveId == title_bar_id)
    {
        ...
        if (held)
        {
            ...
            if (ImGuiTabItem* tab = TabBarFindTabByID(tab_bar, tab_bar->SelectedTabId)) StartMouseMovingWindowOrNode(
                tab->Window,
                node,
                false);
        }
...
static bool DockNodeIsDropAllowedOne(ImGuiWindow* payload, ImGuiWindow* host_window)
{
    if (host_window->DockNodeAsHost && host_window->DockNodeAsHost->IsDockSpace() && payload->BeginOrderWithinContext <
        host_window->BeginOrderWithinContext) return false;
    ...
    if (host_class->ClassId != payload_class->ClassId)
    {
        ...
        if (payload_class->ClassId != 0 && payload_class->DockingAllowUnclassed && host_class->ClassId == 0) return
            true;
        return false;
    }
                                            ...
                                            ImVec2* out_window_menu_button_pos)
{
    ...
    if (node->HasCloseButton) r.Max.x -= g.
        FontSize;// +1.0f; // In DockNodeUpdateTabBar() we currently display a disabled close button even if there is none.
    if (node->HasWindowMenuButton && g.Style.WindowMenuButtonPosition == ImGuiDir_Left) r.Min.x += g.
        FontSize; // + g.Style.ItemInnerSpacing.x; // <-- Adding ItemInnerSpacing makes the title text moves slightly when in a docking tab bar. Instead we adjusted RenderArrowDockMenu()
                                   ...
                                   ImVec2 size_new_desired)
{
    ...
    // Position each node
    if (dir == ImGuiDir_Right || dir == ImGuiDir_Down) pos_new[axis] = pos_old[axis] + size_old[axis] + dock_spacing;
                                                 ...
                                                 ImVec2* test_mouse_pos)
{
    ...
    if (!outer_docking)
    {
        ...
        if (mouse_delta_len2 < r_threshold_sides * r_threshold_sides) return (dir == ImGetDirQuadrantFromDelta(
            mouse_delta.x,
            mouse_delta.y));
    }
                                            ...
                                            bool is_outer_docking)
{
    ...
    if (is_outer_docking) data->IsCenterAvailable = false;
    else if (dst_node_flags & ImGuiDockNodeFlags_NoDocking) data->IsCenterAvailable = false;
        ...
        data->IsCenterAvailable = false;
    else if ((dst_node_flags & ImGuiDockNodeFlags_NoDockingOverMe) || (src_node_flags &
        ImGuiDockNodeFlags_NoDockingOverOther)) data->IsCenterAvailable = false;

    data->IsSidesAvailable = true;
    if ((dst_node_flags & ImGuiDockNodeFlags_NoSplit) || g.IO.ConfigDockingNoSplit) data->IsSidesAvailable = false;
    else if (!is_outer_docking && host_node && host_node->ParentNode == nullptr && host_node->IsCentralNode()) data->
        IsSidesAvailable = false;
    else if ((dst_node_flags & ImGuiDockNodeFlags_NoDockingSplitMe) || (src_node_flags &
        ImGuiDockNodeFlags_NoDockingSplitOther)) data->IsSidesAvailable = false;
    ...
    if (!host_window->Collapsed)
        for (int dir = ImGuiDir_None; dir < ImGuiDir_COUNT; dir++)
        {
            ...
            if (DockNodeCalcDropRectsAndTestMousePos(data->FutureNode.Rect(),
                                                     static_cast<ImGuiDir>(dir),
                                                     data->DropRectsDraw[dir + 1],
                                                     is_outer_docking,
                                                     &g.IO.MousePos))
            {
                data->SplitDir           = static_cast<ImGuiDir>(dir);
                                             ...
                                             const ImGuiDockPreviewData* data)
{
    ...
    if (host_window->Viewport != root_payload->Viewport && !is_transparent_payload) overlay_draw_lists[
        overlay_draw_lists_count++] = GetForegroundDrawList(root_payload->Viewport);
    ...
    if (data->IsDropAllowed)
    {
        ...
        if (data->SplitDir != ImGuiDir_None || data->IsCenterAvailable)
            for (int overlay_n = 0; overlay_n < overlay_draw_lists_count; overlay_n++) overlay_draw_lists[overlay_n]->
                AddRectFilled(overlay_rect.Min, overlay_rect.Max, overlay_col_main, host_window->WindowRounding);
    }
    ...
    if (data->IsDropAllowed && can_preview_tabs && data->SplitDir == ImGuiDir_None && data->IsCenterAvailable)
    {
        ...
        {
            if (!host_node->IsHiddenTabBar() && !host_node->IsNoTabBar()) tab_pos.x += host_node->TabBar->OffsetMax + g.
                Style.ItemInnerSpacing.
                x; // We don't use OffsetNewTab because when using non-persistent-order tab bar it is incremented with each Tab submission.
            else tab_pos.x += g.Style.ItemInnerSpacing.x + TabItemCalcSize(
                host_node->Windows[0]->Name,
                host_node->Windows[0]->HasCloseButton).x;
        }
        else if (!(host_window->Flags & ImGuiWindowFlags_DockNodeHost)) tab_pos.x += g.Style.ItemInnerSpacing.x +
        ...
        for (int payload_n = 0; payload_n < payload_count; payload_n++)
        {
            ...
            for (int overlay_n = 0; overlay_n < overlay_draw_lists_count; overlay_n++)
            {
                ...
                if (!tab_bar_rect.Contains(tab_bb)) overlay_draw_lists[overlay_n]->PushClipRect(
                    tab_bar_rect.Min,
                    tab_bar_rect.Max);
    ...
    for (int dir = ImGuiDir_None; dir < ImGuiDir_COUNT; dir++)
    {
        if (!data->DropRectsDraw[dir + 1].IsInverted())
        {
            ...
            ImU32 overlay_col = (data->SplitDir == static_cast<ImGuiDir>(dir) && data->IsSplitDirExplicit)
            ...
            for (int overlay_n = 0; overlay_n < overlay_draw_lists_count; overlay_n++)
            {
                ...
                overlay_draw_lists[overlay_n]->AddRect(draw_r_in.Min,
                                                       ...
                                                       overlay_col_lines,
                                                       overlay_rounding);
                if (dir == ImGuiDir_Left || dir == ImGuiDir_Right) overlay_draw_lists[overlay_n]->AddLine(
                    ImVec2(center.x, draw_r_in.Min.y),
                    ImVec2(center.x, draw_r_in.Max.y),
                    overlay_col_lines);
                if (dir == ImGuiDir_Up || dir == ImGuiDir_Down) overlay_draw_lists[overlay_n]->AddLine(
...
void ImGui::DockNodeTreeUpdatePosSize(ImGuiDockNode* node, ImVec2 pos, ImVec2 size, bool only_write_to_marked_nodes)
{
    ...
    if (child_0->IsVisible && child_1->IsVisible)
    {
        ...
        const ImGuiAxis axis   = static_cast<ImGuiAxis>(node->SplitAxis);
    ...
    if (child_0->IsVisible && child_1->IsVisible)
    {
        ...
        else
        {
            ...
            if (g.ActiveId == splitter_id)
            {
                ...
                for (int touching_node_n = 0; touching_node_n < touching_nodes[0].Size; touching_node_n++) resize_limits
                    [0] = ImMax(resize_limits[0], touching_nodes[0][touching_node_n]->Rect().Min[axis] + min_size);
                for (int touching_node_n = 0; touching_node_n < touching_nodes[1].Size; touching_node_n++) resize_limits
                    [1] = ImMin(resize_limits[1], touching_nodes[1][touching_node_n]->Rect().Max[axis] - min_size);
...
void ImGui::DockSpace(ImGuiID id,
                      ...
                      const ImGuiWindowClass* window_class)
{
    ...
    if (node->IsLeafNode() && !node->IsCentralNode()) node->LocalFlags |= ImGuiDockNodeFlags_CentralNode;
                                             ...
                                             ImVector<ImGuiID>* out_node_remap_pairs)
{
    ...
    for (int child_n = 0; child_n < IM_ARRAYSIZE(src_node->ChildNodes); child_n++)
        ...
        {
            dst_node->ChildNodes[child_n] = DockBuilderCopyNodeRec(src_node->ChildNodes[child_n],
                                                                   0,
                                     ...
                                     ImVector<const char*>* in_window_remap_pairs)
{
    ...
    for (int remap_window_n = 0; remap_window_n < in_window_remap_pairs->Size; remap_window_n += 2)
    {
        ...
        if (ImGuiWindow* src_window = FindWindowByID(src_window_id)) src_dock_id = src_window->DockId;
        else if (ImGuiWindowSettings* src_window_settings = FindWindowSettings(src_window_id)) src_dock_id =
            src_window_settings->DockId;
...
void ImGui::BeginDocked(ImGuiWindow* window, bool* p_open)
{
    ...
    if (node->LastFrameAlive < g.FrameCount)
    {
        ...
        ImGuiDockNode* root_node = DockNodeGetRootNode(node);
        if (root_node->LastFrameAlive < g.FrameCount) DockContextProcessUndockWindow(ctx, window);
        else
        {
    ...
    if (node->IsHiddenTabBar() || node->IsNoTabBar()) window->Flags |= ImGuiWindowFlags_NoTitleBar;
        ...
        ImGuiWindowFlags_NoTitleBar;      // Clear the NoTitleBar flag in case the user set it: confusingly enough we need a title bar height so we are correctly offset, but it won't be displayed!

    // Save new dock order only if the tab bar has been visible once.
    // This allows multiple windows to be created in the same frame and have their respective dock orders preserved.
    if (node->TabBar && node->TabBar->CurrFrameVisible != -1) window->DockOrder = static_cast<short>(
        DockNodeGetTabOrder(window));

    if ((node->WantCloseAll || node->WantCloseTabId == window->ID) && p_open != nullptr) *p_open = false;
...
void ImGui::BeginDockableDragDropTarget(ImGuiWindow* window)
{
                              ...
                              ImGuiDragDropFlags_AcceptBeforeDelivery | ImGuiDragDropFlags_AcceptNoDrawDefaultRect))
    {
        ...
        if (window->DockNodeAsHost)
        {
            ...
            if (node && node->IsDockSpace() && node->IsRootNode()) node = (node->CentralNode && node->IsLeafNode())
                                                                              ? node->CentralNode
        ...
        else
        {
            ...
            else dock_into_floating_window = true; // Dock into a regular window
        }
        ...
        const bool is_explicit_target = g.IO.ConfigDockingWithShift || IsMouseHoveringRect(
            explicit_target_rect.Min,
        ...
        if (do_preview && (node != nullptr || dock_into_floating_window))
        {
            ...
            if (split_data->IsDropAllowed && payload->IsDelivery()) DockContextQueueDock(
                ctx,
                window,
...
static void ImGui::DockSettingsHandler_ReadLine(ImGuiContext* ctx, ImGuiSettingsHandler*, void*, const char* line)
{
    ...
    if (node.ParentNodeId != 0)
        if (ImGuiDockNodeSettings* parent_settings = DockSettingsFindNodeSettings(ctx, node.ParentNodeId)) node.Depth =
            parent_settings->Depth + 1;
...
static void ImGui::DockSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    int max_depth = 0;
    for (int node_n = 0; node_n < dc->NodesSettings.Size; node_n++) max_depth = ImMax(
        static_cast<int>(dc->NodesSettings[node_n].Depth),
        max_depth);
    ...
    for (int node_n = 0; node_n < dc->NodesSettings.Size; node_n++)
    {
        ...
        if (node_settings->ParentNodeId) buf->appendf(" Parent=0x%08X SizeRef=%d,%d",
        ...
        if (node_settings->SplitAxis != ImGuiAxis_None) buf->appendf(" Split=%c",
                                                                     (node_settings->SplitAxis == ImGuiAxis_X)
                                                                         ? 'X'
                                                                         : 'Y');
...
static void RenderViewportThumbnail(ImDrawList* draw_list, ImGuiViewportP* viewport, const ImRect& bb)
{
    ...
    for (int i = 0; i != g.Windows.Size; i++)
    {
        ...
        ImRect title_r_scaled = ImRect(ImFloor(off + title_r.Min * scale),
                                       ImFloor(off + ImVec2(title_r.Max.x,
                                                            title_r.Min.y) * scale) + ImVec2(
                                           0,
                                           5)); // Exaggerate title bar height
        ...
        const bool window_is_focused = (g.NavWindow && thumb_window->RootWindowForTitleBarHighlight == g.NavWindow->
            RootWindowForTitleBarHighlight);
        window->DrawList->AddRectFilled(thumb_r_scaled.Min,
                                        thumb_r_scaled.Max,
                                        ImGui::GetColorU32(ImGuiCol_WindowBg, alpha_mul));
        window->DrawList->AddRectFilled(title_r_scaled.Min,
                                        title_r_scaled.Max,
                                        ImGui::GetColorU32(
                                            window_is_focused ? ImGuiCol_TitleBgActive : ImGuiCol_TitleBg,
                                            alpha_mul));
        window->DrawList->AddRect(thumb_r_scaled.Min,
                                  thumb_r_scaled.Max,
                                  ImGui::GetColorU32(ImGuiCol_Border, alpha_mul));
        if (ImGuiWindow* window_for_title = GetWindowForTitleDisplay(thumb_window)) window->DrawList->AddText(
            g.Font,
            g.FontSize * 1.0f,
            title_r_scaled.Min,
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
                                                      ...
                                                      bool show_aabb)
        {
            ...
            for (unsigned int base_idx = elem_offset; base_idx < (elem_offset + draw_cmd->ElemCount); base_idx += 3)
            {
                ...
                if (show_mesh) fg_draw_list->
                    AddPolyline(triangle, 3, IM_COL32(255, 255, 0, 255), true, 1.0f); // In yellow: mesh triangles
            }
            // Draw bounding boxes
            if (show_aabb)
            {
                                 ...
                                 const char* label)
        {
            ...
            ImDrawList* fg_draw_list =
                viewport
                    ? GetForegroundDrawList(viewport)
                    : nullptr; // Render additional visuals into the top-most draw list
            if (window && fg_draw_list && IsItemHovered()) fg_draw_list->AddRect(
                window->Pos,
                window->Pos + window->Size,
                IM_COL32(255, 255, 0, 255));
            if (!node_open) return;

            if (window && !window->WasActive) TextDisabled(
            ...
            for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset +=
                 pcmd->ElemCount, pcmd++)
            {
                if (pcmd->UserCallback == nullptr && pcmd->ElemCount == 0) continue;
                if (pcmd->UserCallback)
                {
                ...
                ) NodeDrawCmdShowMeshAndBoundingBox(fg_draw_list,
                                                    draw_list,
                                                    pcmd,
                                                    elem_offset,
                                                    show_drawcmd_mesh,
                ...
                {
                    ImVec2 triangle[3];
                    for (int n = 0; n < 3; n++) triangle[n] = draw_list->VtxBuffer[idx_buffer
                ...
                ImFormatString(buf,
                               ...
                               total_area);
                Selectable(buf);
                if (IsItemHovered() && fg_draw_list) NodeDrawCmdShowMeshAndBoundingBox(
                    fg_draw_list,
                    draw_list,
                    pcmd,
                    elem_offset,
                ...
                ImGuiListClipper
                    clipper(pcmd->ElemCount / 3); // Manually coarse clip our print out of individual vertices to save CPU, only items that may be visible.
                while (clipper.Step())
        ...
        {
            if (!TreeNode((void*)static_cast<uintptr_t>(columns->ID),
                          "Columns Id: 0x%08X, Count: %d, Flags: 0x%04X",
                          columns->ID,
                          columns->Count,
                          columns->Flags)) return;
            BulletText("Width: %.1f (MinX: %.1f, MaxX: %.1f)",
                       columns->OffMaxX - columns->OffMinX,
                       columns->OffMinX,
                       columns->OffMaxX);
            for (int column_n = 0; column_n < columns->Columns.Size; column_n++) BulletText(
        ...
        static void NodeWindow(ImGuiWindow* window, const char* label)
        {
            ...
            if (IsItemHovered() && is_active) GetForegroundDrawList(window)->AddRect(
                window->Pos,
                window->Pos + window->Size,
                IM_COL32(255, 255, 0, 255));
            if (!open) return;
            ...
            if (window->DockNode || window->DockNodeAsHost) NodeDockNode(
        ...
        static void NodeViewport(ImGuiViewportP* viewport)
        {
            ...
            if (TreeNode((void*)static_cast<intptr_t>(viewport->ID),
                         ...
                         viewport->Window ? viewport->Window->Name : "N/A"))
            {
                ...
                for (int layer_i = 0; layer_i < IM_ARRAYSIZE(viewport->DrawDataBuilder.Layers); layer_i++) for (
                    int draw_list_i = 0; draw_list_i < viewport->DrawDataBuilder.Layers[layer_i].Size; draw_list_i++)
                    NodeDrawList(nullptr, viewport, viewport->DrawDataBuilder.Layers[layer_i][draw_list_i], "DrawList");
                TreePop();
            }
        }
        ...
        static void NodeDockNode(ImGuiDockNode* node, const char* label)
        {
            ...
            bool open;
            if (node->Windows.Size > 0) open = TreeNode((void*)static_cast<intptr_t>(node->ID),
            ...
            else open = TreeNode((void*)static_cast<intptr_t>(node->ID),
            ...
            if (is_active && IsItemHovered()) GetForegroundDrawList(
                node->HostWindow ? node->HostWindow : node->VisibleWindow)->AddRect(
                node->Pos,
                node->Pos + node->Size,
                IM_COL32(255, 255, 0, 255));
            if (open)
            {
                ...
                if (TreeNode("flags", "LocalFlags: 0x%04X SharedFlags: 0x%04X", node->LocalFlags, node->SharedFlags))
                {
                    ...
                    CheckboxFlags("LocalFlags: HiddenTabBar",
                                  (ImU32*)&node->LocalFlags,
                                  ImGuiDockNodeFlags_HiddenTabBar);
                    CheckboxFlags("LocalFlags: NoWindowMenuButton",
                                  (ImU32*)&node->LocalFlags,
                                  ImGuiDockNodeFlags_NoWindowMenuButton);
                    CheckboxFlags("LocalFlags: NoCloseButton",
                                  (ImU32*)&node->LocalFlags,
        ...
        static void NodeTabBar(ImGuiTabBar* tab_bar)
        {
            ...
            if (tab_bar->Flags & ImGuiTabBarFlags_DockNode)
            {
                ...
                for (int tab_n = 0; tab_n < ImMin(tab_bar->Tabs.Size, 3); tab_n++) p += ImFormatString(
                    p,
                    buf_end - p,
                    "%s'%s'",
                    tab_n > 0 ? ", " : "",
        ...
        static void NodeStorage(ImGuiStorage* storage, const char* label)
        {
            if (!TreeNode(label,
                          ...
                          label,
                          storage->Data.Size,
                          storage->Data.size_in_bytes())) return;
    ...
    if (TreeNode("Tools"))
    {
        ...
        show_windows_rects |= Combo("##show_windows_rect_type",
    ...
    if (TreeNode("Dock nodes"))
    {
        ...
        Checkbox("Ctrl shows window dock info", &show_docking_nodes);
    ...
    if (TreeNode("Settings"))
    {
        ...
        {
            for (ImGuiWindowSettings* settings = g.SettingsWindows.begin(); settings != nullptr; settings = g.
                 SettingsWindows.next_chunk(settings)) Funcs::NodeWindowSettings(settings);
            TreePop();
        }
        ...
        if (TreeNode("SettingsDocking", "Settings packed data: Docking"))
        {
            ...
            for (ImGuiWindowSettings* settings = g.SettingsWindows.begin(); settings != nullptr; settings = g.
                 SettingsWindows.next_chunk(settings)) if (settings->DockId != 0) BulletText(
                "Window '%s' -> DockId %08X",
                settings->GetName(),
                settings->DockId);
            ...
            for (int n = 0; n < dc->NodesSettings.Size; n++)
            {
                ...
                {
                    if (ImGuiWindow* window = FindWindowByID(settings->SelectedWindowId)) selected_tab_name = window->
                        Name;
        ...
        if (TreeNode("SettingsIniData", "Settings unpacked data (.ini): %d bytes", g.SettingsIniData.size()))
        {
            InputTextMultiline("##Ini",
                               static_cast<char*>((void*)g.SettingsIniData.c_str()),
    ...
    if (show_docking_nodes && g.IO.KeyCtrl && g.HoveredDockNode)
    {
        ...
        ImDrawList* overlay_draw_list = node->HostWindow
                                            ...
                                            : GetForegroundDrawList(static_cast<ImGuiViewportP*>(GetMainViewport()));
        p += ImFormatString(p,

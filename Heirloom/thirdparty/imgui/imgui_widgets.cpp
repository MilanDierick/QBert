        ...
        if (!g.LogEnabled)
        {
            ...
            if (lines_skippable > 0)
            {
                ...
                while (line < text_end && lines_skipped < lines_skippable)
                {
                    ...
                    if ((flags & ImGuiTextFlags_NoWidthForLargeClippedText) == 0) text_size.x = ImMax(
                        text_size.x,
...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
    ...
    if (label_size.x > 0.0f) RenderText(ImVec2(value_bb.Max.x + style.ItemInnerSpacing.x,
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (hovered && g.DragDropActive && g.DragDropPayload.SourceId == id && !(g.DragDropSourceFlags &
        ImGuiDragDropFlags_SourceNoDisableHover)) hovered = false;
    ...
    if (hovered && (flags & ImGuiButtonFlags_AllowItemOverlap) && (g.HoveredIdPreviousFrame != id && g.
        HoveredIdPreviousFrame != 0)) hovered = false;
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos, ImGuiDockNode* dock_node)
{
    ...
    ImU32 text_col = GetColorU32(ImGuiCol_Text);
    ImVec2 center  = bb.GetCenter();
    if (hovered || held) window->DrawList->AddCircleFilled(center + off + ImVec2(0, -0.5f),
                                                           g.FontSize * 0.5f + 1.0f,
                                                           bg_col,
                                                           12);

    if (dock_node) RenderArrowDockMenu(window->DrawList, bb.Min + g.Style.FramePadding, g.FontSize, text_col);
    else RenderArrow(window->DrawList,
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis)
{
    ...
    if (axis == ImGuiAxis_X) return ImRect(inner_rect.Min.x,
                                           ImMax(outer_rect.Min.y, outer_rect.Max.y - border_size - scrollbar_size),
                                           inner_rect.Max.x,
                                           outer_rect.Max.y);
    else return ImRect(ImMax(outer_rect.Min.x, outer_rect.Max.x - border_size - scrollbar_size),
                       inner_rect.Min.y,
                       outer_rect.Max.x,
                       inner_rect.Max.y);
}
                        ...
                        ImDrawCornerFlags rounding_corners)
{
    ...
    float alpha = 1.0f;
    if ((axis == ImGuiAxis_Y) && bb_frame_height < g.FontSize + g.Style.FramePadding.y * 2.0f) alpha = ImSaturate(
    ...
    ImRect grab_rect;
    if (axis == ImGuiAxis_X) grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm),
                                                bb.Min.y,
                                                ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels,
                                                bb.Max.y);
    else grab_rect = ImRect(bb.Min.x,
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (label_size.x > 0.0f) RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x,
                                               check_bb.Min.y + style.FramePadding.y),
                                        label);

    IMGUI_TEST_ENGINE_ITEM_INFO(id,
                                label,
                                window->DC.ItemFlags | ImGuiItemStatusFlags_Checkable | (*v ?
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    if (overlay_size.x > 0.0f) RenderTextClipped(
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview)) window->DrawList->AddRectFilled(
        frame_bb.Min,
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x) RenderArrow(
            window->DrawList,
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview)) RenderTextClipped(
        ...
        NULL,
        ImVec2(0.0f, 0.0f));
    if (label_size.x > 0) RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x,
                                ...
                                const char* format)
{
    ...
    if (data_type == ImGuiDataType_S32 || data_type == ImGuiDataType_U32) return ImFormatString(
        buf,
        buf_size,
        format,
        *(const ImU32*)p_data);
    if (data_type == ImGuiDataType_S64 || data_type == ImGuiDataType_U64) return ImFormatString(
        buf,
        buf_size,
        format,
        *(const ImU64*)p_data);
...
void ImGui::DataTypeApplyOp(ImGuiDataType data_type, int op, void* output, const void* arg1, const void* arg2)
{
    ...
    switch (data_type)
    {
        ...
        case ImGuiDataType_U64: if (op == '+')
            ...
            return;
        case ImGuiDataType_Float: if (op == '+') { *(float*)output = *(const float*)arg1 + *(const float*)arg2; }
            if (op == '-') { *(float*)output = *(const float*)arg1 - *(const float*)arg2; }
            return;
        case ImGuiDataType_Double: if (op == '+') { *(double*)output = *(const double*)arg1 + *(const double*)arg2; }
                                    ...
                                    const char* format)
{
    ...
    else
    {
        ...
        else if (data_type == ImGuiDataType_U8) *(ImU8*)p_data = (ImU8)ImClamp(v32, (int)IM_U8_MIN, (int)IM_U8_MAX);
        else if (data_type == ImGuiDataType_S16) *(ImS16*)p_data = (ImS16)
            ImClamp(v32, (int)IM_S16_MIN, (int)IM_S16_MAX);
        else if (data_type == ImGuiDataType_U16) *(ImU16*)p_data = (ImU16)
...
bool ImGui::DataTypeClamp(ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max)
{
    switch (data_type)
    {
        ...
        case ImGuiDataType_Float: return DataTypeClampT<float
            >((float*)p_data, (const float*)p_min, (const float*)p_max);
        case ImGuiDataType_Double: return DataTypeClampT<double>((double*)p_data,
                                                                 (const double*)p_min,
                                                                 (const double*)p_max);
        case ImGuiDataType_COUNT: break;
    }
                          ...
                          ImGuiSliderFlags flags)
{
    ...
    // Default tweak speed
    if (v_speed == 0.0f && is_clamped && (v_max - v_min < FLT_MAX)) v_speed = (float)((v_max - v_min) * g.
    ...
    if (is_logarithmic)
    {
        ...
        float v_old_parametric = ScaleRatioFromValueT<TYPE, FLOATTYPE>(data_type,
                                                                       v_cur,
                                                                       v_min,
                                                                       v_max,
                                                                       is_logarithmic,
        ...
        v_cur                  = ScaleValueFromRatioT<TYPE, FLOATTYPE>(data_type,
                                                                       v_new_parametric,
                                                                       v_min,
                                                                       v_max,
                                                                       is_logarithmic,
    ...
    if (!(flags & ImGuiSliderFlags_NoRoundToFormat)) v_cur = RoundScalarWithFormatT<TYPE, SIGNEDTYPE>(
        format,
        data_type,
        v_cur);
    ...
    {
        // Convert to parametric space, apply delta, convert back
        float v_new_parametric = ScaleRatioFromValueT<TYPE, FLOATTYPE>(data_type,
                                                                       v_cur,
                                                                       v_min,
                                                                       v_max,
                                                                       is_logarithmic,
                         ...
                         ImGuiSliderFlags flags)
{
    ...
    if ((g.CurrentWindow->DC.ItemFlags & ImGuiItemFlags_ReadOnly) || (flags & ImGuiSliderFlags_ReadOnly)) return false;
                       ...
                       ImGuiSliderFlags flags)
{
    ...
    if (temp_input_is_active)
    {
        ...
        const bool is_clamp_input = (flags & ImGuiSliderFlags_ClampOnInput) != 0 && (p_min == NULL || p_max == NULL ||
    ...
    if (label_size.x > 0.0f) RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x,
                                               frame_bb.Min.y + style.FramePadding.y),
                                        label);
                                  ...
                                  float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        else if (v_clamped >= v_max_fudged) result =
        ...
        else if ((v_min * v_max) < 0.0f) // Range crosses zero, so split into two portions
        {
            ...
            else if (v < 0.0f) result = (1.0f - (float)(ImLog(-(FLOATTYPE)v_clamped / logarithmic_zero_epsilon) / ImLog(
                -v_min_fudged / logarithmic_zero_epsilon))) * zero_point_snap_L;
            else result = zero_point_snap_R + ((float)(ImLog((FLOATTYPE)v_clamped / logarithmic_zero_epsilon) / ImLog(
                v_max_fudged / logarithmic_zero_epsilon)) * (1.0f - zero_point_snap_R));
        }
        else if ((v_min < 0.0f) || (v_max < 0.0f)) // Entirely negative slider
            result = 1.0f - (float)(ImLog(-(FLOATTYPE)v_clamped / -v_max_fudged) / ImLog(-v_min_fudged / -v_max_fudged)
            );
        else result = (float)(ImLog((FLOATTYPE)v_clamped / v_min_fudged) / ImLog(v_max_fudged / v_min_fudged));
    ...
    if (is_logarithmic)
    {
        ...
        else
        {
            ...
            if ((v_min * v_max) < 0.0f) // Range crosses zero, so we have to do this in two parts
            {
                ...
                float zero_point_snap_L = zero_point_center - zero_deadzone_halfsize;
                float zero_point_snap_R = zero_point_center + zero_deadzone_halfsize;
                if (t_with_flip >= zero_point_snap_L && t_with_flip <= zero_point_snap_R) result = (TYPE)
                    0.0f; // Special case to make getting exactly zero possible (the epsilon prevents it otherwise)
                else if (t_with_flip < zero_point_center) result = (TYPE)-(logarithmic_zero_epsilon * ImPow(
                    -v_min_fudged / logarithmic_zero_epsilon,
                    (FLOATTYPE)(1.0f - (t_with_flip / zero_point_snap_L))));
                else result = (TYPE)(logarithmic_zero_epsilon * ImPow(v_max_fudged / logarithmic_zero_epsilon,
                                                                      (FLOATTYPE)((t_with_flip - zero_point_snap_R) / (
                                                                          1.0f - zero_point_snap_R))));
            }
            else if ((v_min < 0.0f) || (v_max < 0.0f)) // Entirely negative slider
                result = (TYPE)-(-v_max_fudged * ImPow(-v_min_fudged / -v_max_fudged, (FLOATTYPE)(1.0f - t_with_flip)));
            else result = (TYPE)(v_min_fudged * ImPow(v_max_fudged / v_min_fudged, (FLOATTYPE)t_with_flip));
        }
    }
    else
    {
        // Linear slider
        if (is_decimal) { result = ImLerp(v_min, v_max, t); }
        else
        {
            ...
            if (v_new_off_floor < v_new_off_round) result = v_min + v_new_off_round;
            else result                                   = v_min + v_new_off_floor;
        }
    }
                            ...
                            ImRect* out_grab_bb)
{
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Nav)
        {
            ...
            if (input_delta != 0.0f)
            {
                ...
                else
                {
                    ...
                    ) input_delta = ((input_delta < 0.0f) ? -1.0f : +1.0f) / (float)
            ...
            {
                clicked_t = ScaleRatioFromValueT<TYPE, FLOATTYPE>(data_type,
                                                                  *v,
                                                                  v_min,
                                                                  v_max,
                                                                  is_logarithmic,
                ...
                else
                {
                    ...
                    TYPE v_new = ScaleValueFromRatioT<TYPE, FLOATTYPE>(data_type,
                                                                       clicked_t,
                                                                       v_min,
                                                                       v_max,
                                                                       is_logarithmic,
                                                                       logarithmic_zero_epsilon,
                                                                       zero_deadzone_halfsize);
                    if (!(flags & ImGuiSliderFlags_NoRoundToFormat)) v_new = RoundScalarWithFormatT<TYPE, SIGNEDTYPE>(
                        format,
                        data_type,
                        v_new);
                    float new_clicked_t = ScaleRatioFromValueT<TYPE, FLOATTYPE>(
                        data_type,
                        v_new,
                        v_min,
                        v_max,
    ...
    else
    {
        ...
        float grab_t = ScaleRatioFromValueT<TYPE, FLOATTYPE>(data_type,
                                                             *v,
                                                             v_min,
                                                             v_max,
                                                             is_logarithmic,
        ...
        if (axis == ImGuiAxis_X) *out_grab_bb = ImRect(grab_pos - grab_sz * 0.5f,
                                                       bb.Min.y + grab_padding,
                                                       grab_pos + grab_sz * 0.5f,
                                                       bb.Max.y - grab_padding);
        else *out_grab_bb = ImRect(bb.Min.x + grab_padding,
                                   grab_pos - grab_sz * 0.5f,
                           ...
                           ImRect* out_grab_bb)
{
    ...
    switch (data_type)
    {
        ...
        case ImGuiDataType_S32: IM_ASSERT(
        ...
        case ImGuiDataType_U32: IM_ASSERT(*(const ImU32*)p_max <= IM_U32_MAX / 2);
            return SliderBehaviorT<ImU32, ImS32, float>(bb,
                                                        ...
                                                        *(const ImU32*)p_max,
                                                        ...
                                                        flags,
                                                        out_grab_bb);
        case ImGuiDataType_S64: IM_ASSERT(
        ...
        case ImGuiDataType_U64: IM_ASSERT(*(const ImU64*)p_max <= IM_U64_MAX / 2);
            return SliderBehaviorT<ImU64, ImS64, double>(bb,
                                                         ...
                                                         *(const ImU64*)p_max,
                                                         ...
                                                         flags,
                                                         out_grab_bb);
        case ImGuiDataType_Float: IM_ASSERT(
            ...
            return SliderBehaviorT<float, float, float>(bb,
                                                        ...
                                                        *(const float*)p_max,
                                                        ...
                                                        flags,
                                                        out_grab_bb);
        case ImGuiDataType_Double: IM_ASSERT(
                         ...
                         ImGuiSliderFlags flags)
{
    ...
    // Render grab
    if (grab_bb.Max.x > grab_bb.Min.x) window->DrawList->AddRectFilled(grab_bb.Min,
                                                                       grab_bb.Max,
                                                                       GetColorU32(
                                                                           g.ActiveId == id
                          ...
                          ImGuiSliderFlags flags)
{
    ...
    // Render grab
    if (grab_bb.Max.y > grab_bb.Min.y) window->DrawList->AddRectFilled(grab_bb.Min,
                                                                       grab_bb.Max,
                                                                       GetColorU32(
                                                                           g.ActiveId == id
                                                                               ? ImGuiCol_SliderGrabActive
                        ...
                        ImGuiInputTextFlags flags)
{
    ...
    bool value_changed = false;
    if ((flags & (ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsScientific)) == 0) flags |=
        ImGuiInputTextFlags_CharsDecimal;
    ...
    {
        if (InputText(label, buf, IM_ARRAYSIZE(buf), flags)) value_changed = DataTypeApplyOpFromText(
            buf,
...
void ImGuiInputTextCallbackData::DeleteChars(int pos, int bytes_count)
{
    ...
    *dst = '\0';
    ...
    if (CursorPos + bytes_count >= pos) CursorPos -= bytes_count;
                        ...
                        void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        draw_window->DC.NavLayerActiveMaskNext |= draw_window->DC.
                                                               NavLayerCurrentMask; // This is to ensure that EndChild() will display a navigation highlight so we can "enter" into it.
    ...
    if (init_state && g.ActiveId != id)
    {
        ...
        state->InitialTextA.
               resize(buf_len + 1);    // UTF-8. we use +1 to make sure that .Data is always pointing to at least an empty string.
        memcpy(state->InitialTextA.Data, buf, buf_len + 1);

        // Start edition
        const char* buf_end = NULL;
        state->TextW.
               resize(buf_size + 1);          // wchar count <= UTF-8 count. we use +1 to make sure that .Data is always pointing to at least an empty string.
    ...
    if (g.ActiveId != id && init_make_active)
    {
        ...
        g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
        if (is_multiline || (flags & ImGuiInputTextFlags_CallbackHistory)) g.ActiveIdUsingNavDirMask |= (1 <<
            ImGuiDir_Up) | (1 << ImGuiDir_Down);
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_startend_key_down = is_osx && io.KeySuper && !io.KeyCtrl && !io.
            KeyAlt;  // OS X style: Line/Text Start and End using Cmd+Arrows instead of Home/End
        ...
        else if (IsKeyPressedMap(ImGuiKey_Backspace) && !is_readonly)
        {
            if (!state->HasSelection())
            {
                ...
                else if (is_osx && io.KeySuper && !io.KeyAlt && !io.KeyCtrl) state->OnKeyPressed(
                ...
                ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ...
                    if (callback_data.BufDirty)
                    {
                        ...
                        if (callback_data.BufTextLen > backup_current_text_length && is_resizable) state->TextW.resize(
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor && state->CursorFollow)
        {
            ...
            if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll))
            {
                ...
                if (cursor_offset.x < state->ScrollX) state->ScrollX = IM_FLOOR(
                    ImMax(0.0f, cursor_offset.x - scroll_increment_x));
                else if (cursor_offset.x - inner_size.x >= state->ScrollX) state->ScrollX = IM_FLOOR(
            ...
            if (is_multiline)
            {
                ...
                else if (cursor_offset.y - inner_size.y >= scroll_y) scroll_y = cursor_offset.y - inner_size.y;
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end;)
            {
                ...
                else
                {
                    ...
                    if (rect_size.x <= 0.0f) rect_size.x =
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) draw_window->DrawList->AddLine(
    ...
    else
    {
        ...
        if (is_multiline) text_size = ImVec2(inner_size.x,
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (flags & ImGuiColorEditFlags_NoInputs) flags = (flags & (~ImGuiColorEditFlags__DisplayMask)) |
    ...
    if ((flags & ImGuiColorEditFlags_InputHSV) && (flags & ImGuiColorEditFlags_DisplayRGB)) ColorConvertHSVtoRGB(
        f[0],
        f[1],
        f[2],
        f[0],
        ...
        ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        const float w_item_one = ImMax(1.0f,
                                       IM_FLOOR(
                                           (w_inputs - (style.ItemInnerSpacing.x) * (components - 1)) / (float)
                                           components));
        ...
        for (int n = 0; n < components; n++)
        {
            ...
            if (!(flags & ImGuiColorEditFlags_NoOptions)) OpenPopupContextItem("context");
        }
    }
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
                      ...
                      ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
        {
            ...
            else sscanf(p, "%02X%02X%02X", (unsigned int*)&i[0], (unsigned int*)&i[1], (unsigned int*)&i[2]);
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions)) OpenPopupContextItem("context");
    }
    ...
    if (value_changed && picker_active_window == NULL)
    {
        ...
        if ((flags & ImGuiColorEditFlags_DisplayRGB) && (flags & ImGuiColorEditFlags_InputHSV)) ColorConvertRGBtoHSV(
            f[0],
            f[1],
            f[2],
            f[0],
        ...
        && BeginDragDropTarget())
    {
        ...
        if (accepted_drag_drop && (flags & ImGuiColorEditFlags_InputHSV)) ColorConvertRGBtoHSV(
            col[0],
            col[1],
            col[2],
            col[0],
    ...
    if (picker_active_window && g.ActiveId != 0 && g.ActiveIdWindow == picker_active_window) window->DC.LastItemId = g.
        ActiveId;
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayHSV || (flags & ImGuiColorEditFlags__DisplayMask) == 0) value_changed |=
            ColorEdit4("##hsv", col, sub_flags | ImGuiColorEditFlags_DisplayHSV);
        if (flags & ImGuiColorEditFlags_DisplayHex || (flags & ImGuiColorEditFlags__DisplayMask) == 0) value_changed |=
    ...
    if (value_changed_fix_hue_wrap && (flags & ImGuiColorEditFlags_InputRGB))
    {
        ...
        if (new_H <= 0 && H > 0)
        {
            if (new_V <= 0 && V != new_V) ColorConvertHSVtoRGB(H,
                                                               S,
    ...
    ImU32 user_col32_striped_of_alpha =
        ColorConvertFloat4ToU32(ImVec4(R,
                                       G,
                                       B,
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        // Render Hue Bar
        for (int i = 0; i < 6; ++i) draw_list->AddRectFilledMultiColor(
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    if (flags & ImGuiColorEditFlags_NoAlpha) flags &= ~(ImGuiColorEditFlags_AlphaPreview |
        ImGuiColorEditFlags_AlphaPreviewHalf);

    ImVec4 col_rgb = col;
    if (flags & ImGuiColorEditFlags_InputHSV) ColorConvertHSVtoRGB(col_rgb.x,
                                                                   col_rgb.y,
                                                                   col_rgb.z,
                                                                   col_rgb.x,
                                                                   col_rgb.y,
    ...
    else
    {
        ...
        if (col_source.w < 1.0f) RenderColorRectWithAlphaCheckerboard(window->DrawList,
                                                                      bb_inner.Min,
                                                                      bb_inner.Max,
                                                                      ...
                                                                      ImVec2(off, off),
                                                                      rounding);
        else window->DrawList->AddRectFilled(bb_inner.Min,
                                             bb_inner.Max,
                                             GetColorU32(col_source),
                                             rounding,
    ...
    {
        if (flags & ImGuiColorEditFlags_NoAlpha) SetDragDropPayload(
    ...
    // Tooltip
    if (!(flags & ImGuiColorEditFlags_NoTooltip) && hovered) ColorTooltip(
        desc_id,
        &col.x,
...
{
    ImGuiContext& g = *GImGui;
    if ((flags & ImGuiColorEditFlags__DisplayMask) == 0) flags |= ImGuiColorEditFlags__OptionsDefault &
    ...
    if ((flags & ImGuiColorEditFlags__PickerMask) == 0) flags |= ImGuiColorEditFlags__OptionsDefault &
        ImGuiColorEditFlags__PickerMask;
    if ((flags & ImGuiColorEditFlags__InputMask) == 0) flags |= ImGuiColorEditFlags__OptionsDefault &
        ImGuiColorEditFlags__InputMask;
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (is_open && !g.NavIdIsAlive && (flags & ImGuiTreeNodeFlags_NavLeftJumpsBackHere) && !(flags &
        ImGuiTreeNodeFlags_NoTreePushOnOpen)) window->DC.TreeJumpToParentOnPopMask |= (1 << window->DC.TreeDepth);
    ...
    else if (flags & ImGuiTreeNodeFlags_OpenOnDoubleClick) button_flags |= ImGuiButtonFlags_PressedOnClickRelease |
    ...
    if (!is_leaf)
    {
        if (pressed && g.DragDropHoldJustPressedId != id)
        {
            if ((flags & (ImGuiTreeNodeFlags_OpenOnArrow | ImGuiTreeNodeFlags_OpenOnDoubleClick)) == 0 || (g.
                NavActivateId == id)) toggled = true;
            if (flags & ImGuiTreeNodeFlags_OpenOnArrow) toggled |= is_mouse_x_over_arrow && !g.
    ...
    if (display_frame)
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet) RenderBullet(window->DrawList,
                                                            ImVec2(text_pos.x - text_offset_x * 0.60f,
                                                                   text_pos.y + g.FontSize * 0.5f),
                                                            text_col);
        else if (!is_leaf) RenderArrow(window->DrawList,
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (size_arg.x == 0.0f || (flags & ImGuiSelectableFlags_SpanAvailWidth)) size.x =
    ...
    if (pressed && (window->Flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiSelectableFlags_DontClosePopups) && !(
        window->DC.ItemFlags & ImGuiItemFlags_SelectableDontClosePopup)) CloseCurrentPopup();
                  ...
                  ImVec2 frame_size)
{
    ...
    // Text overlay
    if (overlay_text) RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y),
                                        frame_bb.Max,
                                        overlay_text,
                                        NULL,
                                        NULL,
                                    ...
                                    float w2) // not using va_arg because they promote float to double
{
    ...
    for (int i = 0; i < IM_ARRAYSIZE(Pos); i++) NextWidth += NextWidths[i] + ((i > 0 && NextWidths[i] > 0.0f)
                                                                                  ? Spacing
                                                                                  : 0.0f);
...
void ImGui::EndMenuBar()
{
    ...
    PopClipRect();
    PopID();
    window->DC.MenuBarOffset.x = window->DC.CursorPos.x - window->MenuBarRect().Min.
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (menuset_is_open) g.NavWindow =
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        window->DC.CursorPos.x +=
            IM_FLOOR(
                style.ItemSpacing.x * (-1.0f + 0.5f)); // -1 spacing to compensate the spacing added when Selectable() did a SameLine(). It would also work to call SameLine() ourselves after the PopStyleVar().
    }
    else
    {
        // Menu inside a menu
        ...
        float min_w = window->DC.MenuColumns.
                              DeclColumns(label_size.x, 0.0f, IM_FLOOR(g.FontSize * 1.20f)); // Feedback to next frame
    ...
    ) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (menu_is_open && !hovered && g.HoveredWindow == window && g.HoveredIdPreviousFrame != 0 && g.
            HoveredIdPreviousFrame != id && !moving_toward_other_child_menu) want_close = true;
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(id,
                                label,
                                window->DC.ItemFlags | ImGuiItemStatusFlags_Openable | (menu_is_open ?
                                    ImGuiItemStatusFlags_Opened : 0));
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    else
    {
        ...
        if (selected) RenderCheckMark(window->DrawList,
...
void ImGui::EndTabBar()
{
    ...
    if (tab_bar->VisibleTabWasSubmitted || tab_bar->VisibleTabId == 0 || tab_bar_appearing) tab_bar->
        LastTabContentHeight    = ImMax(window->DC.CursorPos.y - tab_bar->BarRect.Max.y, 0.0f);
    else window->DC.CursorPos.y = tab_bar->BarRect.Max.y + tab_bar->LastTabContentHeight;
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    if (width_excess > 0.0f && (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown))
    {
        ...
        for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) tab_bar->Tabs[g.ShrinkWidthBuffer[tab_n].Index].Width =
    ...
    if (g.NavWindowingTarget != NULL && g.NavWindowingTarget->DockNode && g.NavWindowingTarget->DockNode->TabBar ==
        tab_bar) tab_bar->VisibleTabId = scroll_track_selected_tab_id = g.NavWindowingTarget->ID;
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    bool want_clip_rect         = !avail_bar_rect.Contains(ImRect(window->DC.CursorPos,
                                                                  window->DC.CursorPos + ImVec2(
                                                                      ...
                                                                      0.0f)));
    if (want_clip_rect) PushClipRect(tab_bar->BarRect.Min,
    ...
    if (ArrowButtonEx("##<",
                      ImGuiDir_Left,
                      arrow_button_size,
                      ImGuiButtonFlags_PressedOnClick | ImGuiButtonFlags_Repeat)) select_dir = -1;
                      ...
                      ImGuiWindow* docked_window)
{
    ...
    if (tab_appearing && (tab_bar->Flags & ImGuiTabBarFlags_AutoSelectNewTabs) && tab_bar->NextSelectedTabId == 0)
        if (!tab_bar_appearing || tab_bar->SelectedTabId == 0) tab_bar->NextSelectedTabId =
    ...
    if (!tab_contents_visible && tab_bar->SelectedTabId == 0 && tab_bar_appearing && docked_window == NULL)
        if (tab_bar->Tabs.Size == 1 && !(tab_bar->Flags & ImGuiTabBarFlags_AutoSelectNewTabs)) tab_contents_visible =
            true;
    ...
    if (want_clip_rect) PushClipRect(ImVec2(ImMax(bb.Min.x, tab_bar->BarRect.Min.x), bb.Min.y - 1),
    ...
    if (g.DragDropActive && !g.DragDropPayload.IsDataType(IMGUI_PAYLOAD_TYPE_WINDOW)) button_flags |=
        ...
        if (docked_window != NULL && !(docked_window->Flags & ImGuiWindowFlags_NoMove))
        {
            ...
            if (!undocking_tab) //&& (!g.IO.ConfigDockingWithShift || g.IO.KeyShift)
            {
                ...
                if (distance_from_edge_y >= threshold_y) undocking_tab = true;
                    ...
                    if ((tab_bar->ReorderRequestDir < 0 && tab_bar->GetTabOrder(tab) == 0) || (tab_bar->
                        ReorderRequestDir > 0 && tab_bar->GetTabOrder(tab) == tab_bar->Tabs.Size - 1)) undocking_tab =
                        true;
            }
    ...
    if (tab_bar->Flags & ImGuiTabBarFlags_NoCloseWithMiddleMouseButton) flags |=
    ...
    if (g.HoveredId == id && !held && g.HoveredIdNotActiveTimer > 0.50f && IsItemHovered())
        if (!(tab_bar->Flags & ImGuiTabBarFlags_NoTooltip) && !(tab->Flags & ImGuiTabItemFlags_NoTooltip)) SetTooltip(
            "%.*s",
...
void ImGui::SetTabItemClosed(const char* label)
{
    ...
    if (is_within_manual_tab_bar)
    {
        ...
        if (ImGuiTabItem* tab = TabBarFindTabByID(tab_bar, tab_id)) tab->WantClose =
...
ImVec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button)
{
    ...
    if (has_close_button) size.x += g.Style.FramePadding.x + (g.Style.ItemInnerSpacing.x + g.FontSize
    ...
    if (close_button_id != 0)
        if (is_contents_visible || bb.GetWidth() >= g.Style.TabMinWidthForUnselectedCloseButton)
            if (g.HoveredId == tab_id || g.HoveredId == close_button_id || g.ActiveId == tab_id || g.ActiveId ==
                close_button_id) close_button_visible = true;
...
static float GetDraggedColumnOffset(ImGuiColumns* columns, int column_index)
{
    ...
    if ((columns->Flags & ImGuiColumnsFlags_NoPreserveWidths)) x = ImMin(
        x,
...
static float GetColumnWidthEx(ImGuiColumns* columns, int column_index, bool before_resize = false)
{
    ...
    float offset_norm;
    if (before_resize) offset_norm = columns->Columns[column_index + 1].OffsetNormBeforeResize - columns->Columns[
...
void ImGui::SetColumnOffset(int column_index, float offset)
{
    ...
    if (!(columns->Flags & ImGuiColumnsFlags_NoForceWithinWindow)) offset = ImMin(
        offset,
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiColumnsFlags_GrowParentContentsSize)) window->DC.CursorMaxPos.x = columns->
    ...
    if (!(flags & ImGuiColumnsFlags_NoBorder) && !window->SkipItems)
    {
        ...
        if (dragging_column != -1)
        {
            if (!columns->IsBeingResized)
                for (int n = 0; n < columns->Count + 1; n++) columns->Columns[n].OffsetNormBeforeResize = columns->
